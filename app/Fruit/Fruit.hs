...
smilingFaceProblem = do
    kspaceMaskValue <- read2DValues (directory ++ "kspaceMask.txt")
    mReValues <- sequence $ [ read2DValues (directory ++ "mRe"++show i++".txt") | i <- [0..numCoils]]
    mImValues <- sequence $ [ read2DValues (directory ++ "mIm"++show i++".txt") | i <- [0..numCoils]]
    --mReValues <- sequence $ [ read2DValues (directory ++ "sRe"++show i++".txt") | i <- [0..numCoils]]
    --mImValues <- sequence $ [ read2DValues (directory ++ "sIm"++show i++".txt") | i <- [0..numCoils]]
    imageMaskValue <- read2DValues (directory ++ "imageMask.txt")
    ...
    let objectiveFunction =
            sum1 [ norm2square ((kMask +: zero) * (ft ((sRe i +: sIm i) * (x +: y)) - (mRe i +: mIm i)))
                 | i <- [0..numCoils]
                 ] +
            ...
            sum1 [ sumElements $ ((sRe i) - rotate (0,1) (sRe i))^2
    ...
    let valMap =
            fromList $
                [ ("kmask", V2D kspaceMaskValue)
                , ("imageMask", V2D imageMaskValue)
                ] ++
                ( zipWith ( \ i vals -> ("mRe"++ show i, V2D vals)) [0..numCoils] mReValues ) ++
                ( zipWith ( \ i vals -> ("mIm"++ show i, V2D vals)) [0..numCoils] mImValues )
        vars = Set.fromList ["x"]
