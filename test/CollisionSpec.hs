...
prop_safeMergeDiffShouldResolveCollision (CollisionSample contextMp accMp diff hashSpace) = do
  results <- replicateM 50 $ do
    ...
    let (mp, nID) = safeMergeDiff dumbHash contextMp accMp diff
    return $ debugPrint (mp `IM.union` contextMp, nID)
  -- Should preserve the diff's semantic regardless of hash scheme
  -- All diffs should refer to the same expression
