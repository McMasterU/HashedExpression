...
spec =
    describe "Simplify spec" $ do
        specify "simplify scalar one zero" $ do
            ...
            simplify (x + y) *
                (x + y) `shouldBe`  simplify ((const 2.0 *. (x * y)) + (x ^ 2) + (y ^ 2))
            simplify (x + y) ^
                2 `shouldBe`  simplify ((const 2.0 *. (x * y)) + (x ^ 2) + (y ^ 2))
            simplify (x - y) ^
                2 `shouldBe` simplify  ((const (-2.0) *. (x * y)) + (x ^ 2) + (y ^ 2))
            simplify (x - y) *
                (x - y) `shouldBe` simplify
                ((const (-2.0) *. (x * y)) + (x ^ 2) + (y ^ 2))
...
---
            ...
            simplify (((x * y) ^ 3) * x + y) `shouldBe` simplify
            ...
            simplify (((x + y) ^ 2) * ((x - y) ^ 2)) `shouldBe` simplify
                ((const (-2.0) *. ((x ^ 2) * (y ^ 2))) + (x ^ 4) + (y ^ 4))
            simplify (((x + y) ^ 2) + ((x - y) ^ 2)) `shouldBe` simplify
                ((const 2.0 *. (x ^ 2)) + (const 2.0 *. (y ^ 2)))
            simplify (((x + y) ^ 2) + ((x - y) ^ 2)) `shouldBe` simplify
                (const 4.0 *. (x * y))
            simplify ((x + y) * (x - y)) `shouldBe` simplify
                ((const (-1.0) *. (y ^ 2)) + (x ^ 2))
...
--DOT PRODUCT
            ...
            simplify (x<.>(x+y))^2 `shouldBe` simplify (((x^2)*(y^2))+(const 2.0*.(y*(x^3)))+(x^4))
