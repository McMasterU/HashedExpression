...
spec = do
  describe "Normalize spec" $ do
    ...
    specify "complex related" $ do
      ((x +: y) * (z +: w))
        `shouldNormalizeTo` ((x * z - y * w) +: (x * w + y * z))
    ...
    specify "distributivity" $ do
      ...
      x
        * sum [y, z, t, u, v]
        `shouldNormalizeTo` sum (map (x *) [y, z, t, u, v])
      sum [y, z, t, u, v]
      ...
      sum [y, z, t, u, v] <.> x
        `shouldNormalizeTo` sum (map (<.> x) [y, z, t, u, v])
      product [a, b, c, sum [x, y, z]]
        `shouldNormalizeTo` sum (map (product . (: [a, b, c])) [x, y, z])
      (x + y) * (z + t) * a
        * b
        `shouldNormalizeTo` (a * b * x * z + a * b * x * t + a * b * y * z + a * b * y * t)
    specify "flatten sum and product" $ do
      product [x * y, product [z, t, w], one]
        `shouldNormalizeTo` product [x, y, z, t, w]
      sum [x + y, sum [z, t, w + s], zero]
        `shouldNormalizeTo` sum [x, y, z, t, w, s]
    specify "group constants together" $ do
      product [one, one, x, y, one, z]
        `shouldNormalizeTo` product [x, y, z]
      sum [one, one, x, y, one, z]
        `shouldNormalizeTo` sum [constant 3, x, y, z]
      product [constant 1, constant 2, x, y, constant 3, z]
        `shouldNormalizeTo` product [constant 6, x, y, z]
    specify "combine same terms" $
      -- Higher dimension this is correct
      do
        sum [one *. x1, x1, x1, constant 3 *. y1, y1]
          `shouldNormalizeTo` sum [constant 3 *. x1, constant 4 *. y1]
        sum [constant (-1) *. x1, x1, constant 3 *. y1, y1, z1]
          `shouldNormalizeTo` sum [constant 4 *. y1, z1]
        x1 - x1 `shouldNormalizeTo` zero1
        sum [one *. x, x, x, constant 3 *. y, y]
          `shouldNormalizeTo` sum [constant 3 *. x, constant 4 *. y]
        sum [constant (-1) *. x, x, constant 3 *. y, y, z]
          `shouldNormalizeTo` sum [constant 4 *. y, z]
  ...
  describe "Normalize piecewise"
    ...
    $ do
      ...
      piecewise [4, 5] x1 [zero1, x1 * (s *. y1), zero1]
        `shouldNormalizeTo` s
        *. piecewise [4, 5] x1 [zero1, x1 * y1, zero1]
      piecewise [1] c1 [c1 +: y1, z1 +: t1]
        `shouldNormalizeTo` piecewise [1] c1 [c1, z1]
        +: piecewise [1] c1 [y1, t1]
  describe "Fourier transform"
    $ specify "some Ft rules"
    $ do
      reFT (reFT x1)
        + imFT (imFT x1)
        `shouldNormalizeTo` constant (fromIntegral defaultDim1D)
        *. x1
      reFT (reFT x2)
        + imFT (imFT x2)
        `shouldNormalizeTo` constant (fromIntegral (default1stDim2D * default2ndDim2D))
        *. x2
