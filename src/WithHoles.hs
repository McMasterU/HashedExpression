...
import HashedExpression (Internal,Complex,RealVectorSpace,Rectangular,Node,getDimE
                        ,ExpressionEdge(..),addEdge,Dir(..),OpId(..)
                        ,Dims(..))
...
data WithHoles  = WHOp OpId [WithHoles]
                | WHConst Double
...
isMatch exprs (WHOp opW xs) n
  ...
  , not $ any (== Nothing) subMatches
  = Just $ concat $ catMaybes subMatches
...
dimWH found e0 (WHOp op xs@(_x1:_)) =
  case op of
    ...
    _         -> case catMaybes $ map (dimWH found e0) xs of
...
apply dims found e0 (WHOp op xs) = addEdge e1 (Op dims op ns)
  where
    (e1,ns) = case op of
                ScaleV -> case xs of
                            [xS,xV] -> let (e1,nS) = apply Dim0 found e0 xS
                                           (e2,nV) = apply dims found e1 xV
                                       in (e2,[nS,nV])
                            _            -> error $ "WH.apply Scale "++show (dims,found,e0,op,xs)
...
applyOne (e,n) ((GP pattern condition,replacement):rules)
  ...
  , condition e found
  = Just $ apply (getDimE e n) found e replacement
  | otherwise
  = applyOne (e,n) rules
applyOne _ [] = Nothing
{-
...
containsDifferential (WHHole i) pattern =
  ...
  then \ exprs holes -> case lookup i holes of
...
(&&.) :: Condition -> Condition -> Condition
(&&.) = mergeWith (&&)
mergeWith op c1 c2 = \ pattern -> let c1Ok = c1 pattern
                                      c2Ok = c2 pattern
