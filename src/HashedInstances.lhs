...
instance Complex ScalarC Scalar where
  (Scalar reExpr) +: (Scalar imExpr) = let
    ...
    in
      ScalarC $ addEdge' es (Op Dim0 RealImag [reN,imN])
  iRe (Scalar (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const Dim0 0)
    in
        ScalarC $ addEdge' exprs1 (Op Dim0 RealImag [n,n0])
  iIm (Scalar (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const Dim0 0)
    in
...
instance Fractional Scalar where
    (Scalar expr1) / (Scalar expr2) =
         let (Expression _ es,(n1,n2)) = merge expr1 expr2
         in case (maybeConst expr1, maybeConst expr2) of
           (_, Just 0) -> error $ "divide by zero constant in " ++ show (expr1,expr2,es)
...
instance Floating Scalar where
    ...
    exp (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ exp x)
           _ -> addEdge' es (Op Dim0 Exp [n])

    cos (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ cos x)
           _ -> addEdge' es (Op Dim0 Cos [n])

    sin (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ sin x)
           _ -> addEdge' es (Op Dim0 Sin [n])

    tan (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ tan x)
           _ -> addEdge' es (Op Dim0 Tan [n])

    log (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ log x)
           _ -> addEdge' es (Op Dim0 Log [n])

    asin (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ asin x)
           _ -> addEdge' es (Op Dim0 Asin [n])

    acos (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ acos x)
           _ -> addEdge' es (Op Dim0 Acos [n])

    atan (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ atan x)
           _ -> addEdge' es (Op Dim0 Atan [n])

    sinh (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ sinh x)
           _ -> addEdge' es (Op Dim0 Sinh [n])

    cosh (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ cosh x)
           _ -> addEdge' es (Op Dim0 Cosh [n])

    tanh (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ atanh x)
           _ -> addEdge' es (Op Dim0 Tanh [n])

    atanh (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
           Just x -> addEdge' es (Const Dim0 $ atanh x)
           _ -> addEdge' es (Op Dim0 Atanh [n])

    acosh (Scalar expr1@(Expression n es)) = Scalar $ case maybeConst expr1 of
...
instance RealVectorSpace OneD Scalar where
  ...
  dot (OneD v1) (OneD v2) = let (Expression _ es,(n1,n2)) = merge v1 v2
    in  if getDimE es n1 == getDimE es n2
        then Scalar $ addEdge' es (Op Dim0 Dot [n1,n2])
        else error $ "dot 1D sizes differ "

  subMask (OneD v1) (OneD v2) = let (Expression _ es,(n1,n2)) = merge v1 v2
    in  if getDimE es n1 == getDimE es n2
        then OneD $ addEdge' es (Op (getDimE es n1) SubMask [n1,n2])
        else error $ "subMask 1D sizes differ " ++ show (getDimE es n1, getDimE es n2)

  negMask (OneD v1) (OneD v2) = let (Expression _ es,(n1,n2)) = merge v1 v2
    in  if getDimE es n1 == getDimE es n2
...
instance RealVectorSpace TwoD Scalar where
  ...
  dot (TwoD v1) (TwoD v2) = let (Expression _ es,(n1,n2)) = merge v1 v2
    in  if getDimE es n1 == getDimE es n2
        then Scalar $ addEdge' es (Op Dim0 Dot [n1,n2])
        else error $ "dot 1D sizes differ " ++ show (v1,v2)

  subMask (TwoD v1) (TwoD v2) = let (Expression _ es,(n1,n2)) = merge v1 v2
    in  if getDimE es n1 == getDimE es n2
        then TwoD $ addEdge' es (Op (getDimE es n1) SubMask [n1,n2])
        else error $ "subMask 1D sizes differ " ++ show (getDimE es n1, getDimE es n2)

  negMask (TwoD v1) (TwoD v2) = let (Expression _ es,(n1,n2)) = merge v1 v2
    in  if getDimE es n1 == getDimE es n2
...
--}
  projSS ss (TwoD (Expression n e)) = TwoD eOut
    where
      ...
      outDims = case ss of
                  SSCrop _bounds dims -> if dims == [d1,d2] then projDim inDim ss
                                                            else error $ "HI.projSS adding "++show (d1,d2,ss) ++ "\n" ++pretty (e,n)
...
instance RealVectorSpace TwoDSparse Scalar where
  ...
  dot (TwoDSparse sampleList1 v1) (TwoDSparse sampleList2 v2)
    = let (Expression _ es,(n1,n2)) = merge v1 v2
      in  if getDimE es n1 == getDimE es n2 && sampleList1 == sampleList2
          then Scalar $ addEdge' es (Op Dim0 Dot [n1,n2])
          else error $ "dot 2d sparse sizes" ++ show (getDimE es n1,getDimE es n2 ,sampleList1,sampleList2)
  ...
  mapR fun (TwoDSparse sL@(SparseList2D _ _ _ _) (Expression n e)) = TwoDSparse sL eOut
    where
      eOut = addEdge' e $ Op (getDimE e n) (MapND mapOut $ C.pack "anonymous") [n]
      Scalar mapOut = fun $ var "anonymous"
\end{code}
...
instance SubsampledSpace TwoD TwoDSparse SparseList2D where
  projSparse sL@(SparseList2D len skipDim _sL2 (min2,max2)) (TwoD (Expression n e))
    =  let (ok,outDims) = case (skipDim,getDimE e n) of
            (Dii1,Dim2 dd@(d,_)) -> (min2 >= 0 && max2 < d,Dim2SL1 dd len)
            (Dii2,Dim2 dd@(_,d)) -> (min2 >= 0 && max2 < d,Dim2SL2 dd len)
            dd -> error $ "projSparse expected two dimensions " ++ show dd
       in if ok then TwoDSparse sL $ addEdge' e $ Op outDims (Project (SSList2D sL)) [n]
       else error $ "projSparse bad sampleList "++show (getDimE e n,sL)
  injectSparse (TwoDSparse sL@(SparseList2D _len sparseDim _sL2 (min2,max2)) (Expression n e))
    =  let (ok,outDims) = case getDimE e n of
             Dim2SL1 dd@(d,_) _ -> (sparseDim == Dii2 && min2 >= 0 && max2 < d,Dim2 dd)
             Dim2SL2 dd@(_,d) _ -> (sparseDim == Dii1 && min2 >= 0 && max2 < d,Dim2 dd)
...
instance SubsampledSpace ThreeD ThreeDSparse SparseList3D where
  projSparse sL@(SparseList3D len skipDim _sL2 (min2,max2) _sL3 (min3,max3)) (ThreeD (Expression n e))
    =  let (ok,outDims) = case (skipDim,getDimE e n) of
            (Diii1,Dim3 dd@(_,d2,d3)) -> (min2 >= 0 && max2 < d2 && min3 >= 0 && max3 < d3 ,Dim3SL1 dd len)
            (Diii2,Dim3 dd@(d3,_,d2)) -> (min2 >= 0 && max2 < d2 && min3 >= 0 && max3 < d3,Dim3SL2 dd len)
            (Diii3,Dim3 dd@(d2,d3,_)) -> (min2 >= 0 && max2 < d2 && min3 >= 0 && max3 < d3,Dim3SL3 dd len)
            dd -> error $ "projSparse expected three dimensions " ++ show dd
       in if ok then ThreeDSparse sL $ addEdge' e $ Op outDims (Project (SSList3D sL)) [n]
       else error $ "projSparse bad sampleList "++show (getDimE e n,sL)
  injectSparse (ThreeDSparse sL@(SparseList3D _len sparseDim _sL2 (min2,max2) _sL3 (min3,max3)) (Expression n e))
    =  let (ok,outDims) = case (sparseDim,getDimE e n) of
             (Diii1,Dim3SL1 dd@(_,d2,d3) _) -> (min2 >= 0 && max2 < d2 && min3 >= 0 && max3 < d3,Dim3 dd)
             (Diii2,Dim3SL2 dd@(d3,_,d2) _) -> (min2 >= 0 && max2 < d2 && min3 >= 0 && max3 < d3,Dim3 dd)
             (Diii3,Dim3SL3 dd@(d2,d3,_) _) -> (min2 >= 0 && max2 < d2 && min3 >= 0 && max3 < d3,Dim3 dd)
...
instance RealVectorSpace ThreeD Scalar where
  ...
  subMask (ThreeD v1) (ThreeD v2) = let (Expression _ es,(n1,n2)) = merge v1 v2
    in  if getDimE es n1 == getDimE es n2
        then ThreeD $ addEdge' es (Op (getDimE es n1) SubMask [n1,n2])
        else error $ "subMask 1D sizes differ " ++ show (getDimE es n1, getDimE es n2)

  negMask (ThreeD v1) (ThreeD v2) = let (Expression _ es,(n1,n2)) = merge v1 v2
    in  if getDimE es n1 == getDimE es n2
...
instance RealVectorSpace OneDC Scalar where
  ...
  subMask v1 v2
    = (subMask (xRe v1) (xRe v2)) +: (subMask (xIm v1) (xIm v2))
  negMask v1 v2
    = (negMask (xRe v1) (xRe v2)) +: (negMask (xIm v1) (xIm v2))
...
instance (RealVectorSpace v s, Num s, Show v) => RealVectorSpace [v] s where
  ...
  dot xs ys = if length xs == length ys then sum $ zipWith dot xs ys
...
instance RealVectorSpace ThreeDSparse Scalar where
  ...
  mapR fun (ThreeDSparse sL (Expression n e))
      = ThreeDSparse sL eOut
    where
...
instance Num OneD where
  ...
  (OneD expr1) + (OneD expr2) =
    ...
    in if getDimE' expr1 == getDimE' expr2
       ...
       else error $ "can't add 1d vectors of lengths " ++ show (expr1,expr2)
...
instance Complex OneDC OneD where
  reO@(OneD reExpr) +: imO@(OneD imExpr) =  let
      (Expression _ es,(reN,imN)) = merge reExpr imExpr
    in
      if getDimE' reExpr == getDimE' imExpr
        then OneDC $ addEdge' es (Op (getDimE' reExpr) RealImag [reN,imN])
        else error $ "HI.Complex OneDC +: dimension mismatch " ++ show (reO,imO)

  iRe (OneD (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
        OneDC $ addEdge' exprs1 (Op (getDimE exprs n) RealImag [n,n0])

  iIm (OneD (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
        OneDC $ addEdge' exprs1 (Op (getDimE exprs n) RealImag [n0,n])

  xRe (OneDC (Expression n exprs)) = OneD $ addEdge' exprs (Op (getDimE exprs n) RealPart [n])
...
instance Num OneDC where
  ...
  (OneDC expr1) + (OneDC expr2) =
       ...
       in if getDimE' expr1 == getDimE' expr2
          ...
          else error $ "can't add 1d complex vectors of lengths " ++ show (expr1,expr2)
...
instance Num TwoD where
  ...
  (TwoD expr1) + (TwoD expr2) =
     ...
     in if getDimE' expr1 == getDimE' expr2
        ...
        else error $ "can't add 2d vectors of lengths " ++ show (expr1,expr2)
...
instance Complex TwoDC TwoD where
  reO@(TwoD reExpr) +: imO@(TwoD imExpr) =  let
    ...
    in
      if getDimE' reExpr == getDimE' imExpr
        then TwoDC $ addEdge' es (Op (getDimE' reExpr) RealImag [reN,imN])
        else error $ "HI.Complex TwoDC +: dimension mismatch " ++ show (reO,imO)
  iRe (TwoD (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
        TwoDC $ addEdge' exprs1 (Op (getDimE exprs n) RealImag [n,n0])
  iIm (TwoD (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
...
instance Complex TwoDCSparse TwoDSparse where
  reO@(TwoDSparse sLre@(SparseList2D _len _ _ _) reExpr)
    ...
    in
      if getDimE' reExpr == getDimE' imExpr
        then TwoDCSparse sLre $ addEdge' es (Op (getDimE' reExpr) RealImag [reN,imN])
        else error $ "HI.Complex TwoDCSparse +: dimension mismatch " ++ show (reO,imO,sLre,sLim)
  iRe (TwoDSparse sL@(SparseList2D _len _ _ _) (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
        TwoDCSparse sL $ addEdge' exprs1 (Op (getDimE exprs n) RealImag [n,n0])
  iIm (TwoDSparse sL@(SparseList2D _len _ _ _) (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
        TwoDCSparse sL $ addEdge' exprs1 (Op (getDimE exprs n) RealImag [n0,n])
...
instance Complex ThreeDCSparse ThreeDSparse where
  reO@(ThreeDSparse sLre@(SparseList3D _len _ _ _ _ _) reExpr)
    ...
    in
      if getDimE' reExpr == getDimE' imExpr
        then ThreeDCSparse sLre $ addEdge' es (Op (getDimE' reExpr) RealImag [reN,imN])
        else error $ "HI.Complex ThreeDSparse +: dimension mismatch " ++ show (reO,imO,sLre,sLim)
  iRe (ThreeDSparse sL@(SparseList3D _len _ _ _ _ _) (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
        ThreeDCSparse sL $ addEdge' exprs1 (Op (getDimE exprs n) RealImag [n,n0])
  iIm (ThreeDSparse sL@(SparseList3D _len _ _ _ _ _) (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
        ThreeDCSparse sL $ addEdge' exprs1 (Op (getDimE exprs n) RealImag [n0,n])
...
instance ComplexVectorSpace OneDC OneD ScalarC Scalar where
  ...
  realPartV (OneDC (Expression n exprs)) = OneD $ addEdge' exprs (Op (getDimE exprs n) RealPart [n])
...
instance ComplexVectorSpace TwoDC TwoD ScalarC Scalar where
  ...
  realPartV (TwoDC (Expression n exprs)) = TwoD $ addEdge' exprs (Op (getDimE exprs n) RealPart [n])

  imagPartV (TwoDC (Expression n exprs)) = TwoD $ addEdge' exprs (Op (getDimE exprs n) ImagPart [n])
...
instance ComplexVectorSpace TwoDCSparse TwoDSparse ScalarC Scalar where
  ...
  realPartV (TwoDCSparse sL (Expression n exprs)) = TwoDSparse sL
    $ addEdge' exprs (Op (getDimE exprs n) RealPart [n])

  imagPartV (TwoDCSparse sL (Expression n exprs)) = TwoDSparse sL
...
instance Num TwoDCSparse where
    negate (TwoDCSparse sL (Expression n exprs)) = TwoDCSparse sL $
      ...
      where
        (exprs',neg) = addEdge exprs (Const Dim0 (-1))
    fromInteger x = error $ "can't promote integer to 2d sparse vector "++ show x
    (TwoDCSparse sL1 expr1) + (TwoDCSparse sL2 expr2) =
         let (Expression _ es,(n1,n2)) = merge expr1 expr2
         in if getDimE' expr1 == getDimE' expr2
            then TwoDCSparse sL1 $ addEdge' es (Op (getDimE' expr1) Sum [n1,n2])
            else error $ "can't add 2d complex vectors of lengths " ++ show (sL1,sL2)
...
instance Num TwoDC where
    negate (TwoDC (Expression n exprs)) = TwoDC $
      ...
      where
        (exprs',neg) = addEdge exprs (Const Dim0 (-1))
    fromInteger x = error $ "can't promote integer to 2d complex vector "++ show x
...
instance Num ThreeDCSparse where
    negate (ThreeDCSparse sL (Expression n exprs)) = ThreeDCSparse sL
      ...
      where
          (exprs',neg) = addEdge exprs (Const Dim0 (-1))
    fromInteger x = error $ "can't promote integer to 3d complex sparse vector "++ show x
    (ThreeDCSparse sL1 expr1) + (ThreeDCSparse sL2 expr2) =
         let (Expression _ es,(n1,n2)) = merge expr1 expr2
         in if getDimE' expr1 == getDimE' expr2 && sL1 == sL2
            then ThreeDCSparse sL1 $ addEdge' es (Op (getDimE' expr1) Sum [n1,n2])
...
instance Complex ThreeDC ThreeD where
  reO@(ThreeD reExpr) +: imO@(ThreeD imExpr) =  let
    ...
    in
      if getDimE' reExpr == getDimE' imExpr
        then ThreeDC $ addEdge' es (Op (getDimE' reExpr) RealImag [reN,imN])
        else error $ "HI.Complex ThreeDC +: dimension mismatch " ++ show (reO,imO)
  iRe (ThreeD (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
        ThreeDC $ addEdge' exprs1 (Op (getDimE exprs n) RealImag [n,n0])
  iIm (ThreeD (Expression n exprs)) = let
         Expression n0 exprs1 = addEdge' exprs (Const (getDimE exprs n) 0)
    in
...
instance Num ThreeDC where
    ...
    fromInteger x = error $ "can't promote integer to 3d complex vector "++ show x
    (ThreeDC expr1) + (ThreeDC expr2) =
         let (Expression _ es,(n1,n2)) = merge expr1 expr2
         in if getDimE' expr1 == getDimE' expr2
            then ThreeDC $ addEdge' es (Op (getDimE' expr1) Sum [n1,n2])
            else error $ "can't add 3d complex vectors of lengths " ++ show (expr1,expr2)
    _ * _ = error "can't multiply 3d complex vectors"
    abs _ = error "can't abs 3d complex vectors"
    signum _ = error "can't signum 3d complex vectors"
\end{code}
...
instance ShowHex Scalar where
  showHex (Scalar e@(Expression node exprs)) = case I.lookup node exprs of
    ...
    Just (Var dims name) -> case dims of
      Dim0 -> prettyHex e
      _ -> error $ "showHex Scalar got non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim0 -> prettyHex e
      _ -> error $ "showHex Scalar got non-scalar "++ show (Var dims name)
    Just (Op dims op _inputs) -> case (dims,op) of
      (Dim0,_) -> prettyHex e
      _ -> error $ "showHex Scalar got non-scalar " ++ show (dims,op)
    Just (Const dims d) -> case dims of
...
instance Show Scalar where
  show (Scalar e@(Expression node exprs)) = case I.lookup node exprs of
    ...
    Just (Var dims name) -> case dims of
      Dim0 -> pretty e
      _ -> error $ "show Scalar got non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim0 -> pretty e
      _ -> error $ "show Scalar got non-scalar "++ show (Var dims name)
    Just (Op dims op _inputs) -> case (dims,op) of
      (Dim0,_) -> pretty e
      _ -> error $ "show Scalar got non-scalar " ++ show (dims,op)
    Just (Const dims _) -> case dims of
      Dim0 -> pretty e
      DimUnknown -> (pretty e)++(if skipDebug then "" else "?")
      _ -> error $ "show Scalar has dims ! "++show dims
    Just _ -> pretty e

instance Show ScalarC where
  show (ScalarC e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show Scalar found no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim0 -> pretty e
      _ -> error $ "show Scalar got non-scalar "++ show (Var dims name)
...
instance Show OneD where
  show (OneD e@(Expression node exprs)) = case I.lookup node exprs of
    ...
    Just (DVar dims name) -> case dims of
      Dim1 _ -> pretty e
      _ -> error $ "show OneD non-scalar "++ show (DVar dims name)
    Just (Op dims op _inputs) -> case dims of
      Dim1 _ -> pretty e
...
instance Show OneDC where
  show (OneDC e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show OneD no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim1 _ -> pretty e
      _ -> error $ "show OneDC non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim1 _ -> pretty e
      _ -> error $ "show OneDC non-scalar "++ show (Var dims name)
    Just (Op dims op _inputs) -> case dims of
...
instance Show TwoD where
  show (TwoD e@(Expression node exprs)) = case I.lookup node exprs of
...
instance Show TwoDC where
  show (TwoDC e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show TwoDC no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim2 _ -> pretty e
      _ -> error $ "show TwoDC non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim2 _ -> pretty e
      _ -> error $ "show TwoDC non-scalar "++ show (Var dims name)
    Just (Op dims op _inputs) -> case dims of
...
instance Show TwoDSparse where
  show (TwoDSparse (SparseList2D _len _ _ _) e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show TwoDSparse no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim2SL1 _ _ -> pretty e
      Dim2SL2 _ _ -> pretty e
      _ -> error $ "show TwoDSparse non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim2SL1 _ _ -> pretty e
      Dim2SL2 _ _ -> pretty e
      _ -> error $ "show TwoDSparse non-scalar "++ show (Var dims name)
    Just (Op dims op _inputs) -> case dims of
      Dim2SL1 _ _ -> pretty e
      Dim2SL2 _ _ -> pretty e
      _ -> error $ "show TwoDSparse non-scalar " ++ show (dims,op)
    Just (Const dims _) -> case dims of
      Dim2SL1 _ _ -> pretty e
      Dim2SL2 _ _ -> pretty e
...
instance Show TwoDCSparse where
  show (TwoDCSparse (SparseList2D _len _ _ _) e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show TwoDCSparse no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim2SL1 _ _ -> pretty e
      Dim2SL2 _ _ -> pretty e
      _ -> error $ "show TwoDCSparse non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim2SL1 _ _ -> pretty e
      Dim2SL2 _ _ -> pretty e
      _ -> error $ "show TwoDCSparse non-scalar "++ show (Var dims name)
    Just (Op dims op _inputs) -> case dims of
      Dim2SL1 _ _ -> pretty e
      Dim2SL2 _ _ -> pretty e
      _ -> error $ "show TwoDCSparse non-scalar " ++ show (dims,op)
    Just (Const dims _) -> case dims of
      Dim2SL1 _ _ -> pretty e
      Dim2SL2 _ _ -> pretty e
...
instance Show ThreeD where
  show (ThreeD e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show ThreeD no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim3 _ -> pretty e
      _ -> error $ "show ThreeD non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim3 _ -> pretty e
      _ -> error $ "show ThreeD non-scalar "++ show (Var dims name)
    Just (Op dims op _inputs) -> case dims of
...
instance Show ThreeDC where
  show (ThreeDC e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show ThreeDC no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim3 _ -> pretty e
      _ -> error $ "show ThreeDC non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim3 _ -> pretty e
      _ -> error $ "show ThreeDC non-scalar "++ show (Var dims name)
    Just (Op dims op _inputs) -> case dims of
...
instance Show ThreeDSparse where
  show (ThreeDSparse (SparseList3D _len _ _ _ _ _) e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show ThreeDSparse no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim3SL1 _ _ -> pretty e
      Dim3SL2 _ _ -> pretty e
      Dim3SL3 _ _ -> pretty e
      _ -> error $ "show ThreeDSparse non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim3SL1 _ _ -> pretty e
      Dim3SL2 _ _ -> pretty e
      Dim3SL3 _ _ -> pretty e
    ...
    Just (Op dims op _inputs) -> case dims of
      ...
      Dim3SL2 _ _ -> pretty e
      Dim3SL3 _ _ -> pretty e
      _ -> error $ "show ThreeDSparse non-scalar " ++ show (dims,op)
    Just (Const dims _) -> case dims of
      Dim3SL1 _ _ -> pretty e
      Dim3SL2 _ _ -> pretty e
      Dim3SL3 _ _ -> pretty e
...
instance Show ThreeDCSparse where
  show (ThreeDCSparse (SparseList3D _len _ _ _ _ _) e@(Expression node exprs)) = case I.lookup node exprs of
    Nothing -> "show ThreeDCSparse no node " ++ (take 1000 $ show (node,exprs))
    Just (Var dims name) -> case dims of
      Dim3SL1 _ _ -> pretty e
      Dim3SL2 _ _ -> pretty e
      Dim3SL3 _ _ -> pretty e
      _ -> error $ "show ThreeDCSparse non-scalar "++ show (Var dims name)
    Just (DVar dims name) -> case dims of
      Dim3SL1 _ _ -> pretty e
      Dim3SL2 _ _ -> pretty e
      Dim3SL3 _ _ -> pretty e
    ...
    Just (Op dims op _inputs) -> case dims of
      ...
      Dim3SL2 _ _ -> pretty e
      Dim3SL3 _ _ -> pretty e
      _ -> error $ "show ThreeDCSparse non-scalar " ++ show (dims,op)
    Just (Const dims _) -> case dims of
      Dim3SL1 _ _ -> pretty e
      Dim3SL2 _ _ -> pretty e
      Dim3SL3 _ _ -> pretty e
...
prettyAll (e,n) = unlines $ pa [] [n]
  where
    pa _ [] = []
    pa visited (next:tovisit) =  (("----- " ++ show next ++ "   sized " ++ show (getDimE e next))
                                 :("      " ++ pretty' (e,next))
                                 : case I.lookup next e of
                                     Just (Op _ _ args) -> pa (next : visited)
...
instance Rectangular OneDC where
  ...
  invFt (OneDC (Expression n exprs)) = OneDC $
      addEdge' exprs (Op (getDimE exprs n) (FT False) [n])
{--
...
--}
  rowPFT (OneDC (Expression n exprs)) = OneDC $
      addEdge' exprs (Op (getDimE exprs n) (PFT True Row) [n])
  ...
  invRowPFT (OneDC (Expression n exprs)) = OneDC $
...
instance Rectangular TwoDC where
  ...
  invFt (TwoDC (Expression n exprs)) =
    let
      ...
      (e'',s) = addEdge e' $ Const Dim0 $ 1 / fromIntegral scale
    in
      TwoDC $ addEdge' e'' $ Op (getDimE exprs n) (ScaleV) [s,n']
  rowPFT (TwoDC (Expression n exprs)) = TwoDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT True Row) [n])
  colPFT dir (TwoDC (Expression n exprs)) = TwoDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT dir Column) [n])
  columnPFT (TwoDC (Expression n exprs)) = TwoDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT True Column) [n])
  slicePFT _e = error "slicePFT not defined for TwoDC"
  invRowPFT (TwoDC (Expression n exprs)) = TwoDC $                -- FIXME: do we do the 1/dims for for the partial??
    addEdge' exprs (Op (getDimE exprs n) (PFT False Row) [n])
  invColumnPFT (TwoDC (Expression n exprs)) = TwoDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT False Column) [n])
...
instance Rectangular ThreeDC where
  ...
  invFt (ThreeDC (Expression n exprs)) =
    let
      ...
      (e'',s) = addEdge e' $ Const Dim0 $ 1 / fromIntegral scale
    in
      ThreeDC $ addEdge' e'' $ Op (getDimE exprs n) (ScaleV) [s,n']
  rowPFT (ThreeDC (Expression n exprs)) = ThreeDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT True Row) [n])
  colPFT dir (ThreeDC (Expression n exprs)) = ThreeDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT dir Column) [n])
  columnPFT (ThreeDC (Expression n exprs)) = ThreeDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT True Column) [n])
  slicePFT (ThreeDC (Expression n exprs)) = ThreeDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT True Slice) [n])
  invRowPFT (ThreeDC (Expression n exprs)) = ThreeDC $                -- FIXME: do we do the 1/dims for for the partial??
    addEdge' exprs (Op (getDimE exprs n) (PFT False Row) [n])
  invColumnPFT (ThreeDC (Expression n exprs)) = ThreeDC $
    addEdge' exprs (Op (getDimE exprs n) (PFT False Column) [n])
  invSlicePFT (ThreeDC (Expression n exprs)) = ThreeDC $
...
helpRTwoD :: RangeKernel -> [((Int,Int),Double)] -> (Internal,[Node]) -> Scalar
...
helpRThreeD :: RangeKernel -> [((Int,Int,Int),Double)] -> (Internal,[Node]) -> Scalar
