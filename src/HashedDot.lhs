...
threeDC2dot  name  (ThreeDC (Expression n e)) = writeFile (addDot  name ) $ snd $ expr2dot' name (e,n)
threeD2dot  name  (ThreeD (Expression n e)) = writeFile (addDot  name ) $ snd $ expr2dot' name (e,n)
scalar2dot  name  (Scalar (Expression n e)) = writeFile (addDot  name ) $ snd $ expr2dot' name (e,n)
scalar2polys  name  (Scalar (Expression n e)) = fst $ expr2dot' name (e,n)
addDot fileName = case reverse fileName of
                    't':'o':'d':'.':_ -> fileName
                    _                 -> fileName ++ ".dot"
expr2dot' name (e,n) = (sczs,
  ...
  where
    ...
    ((_,sczs),dotDefs) = node2dot (I.fromList $ zip [0..] $ map snd nodeSubs) e (Set.empty,[]) n
    ((_,_notes),nodeSubs) = L.mapAccumL mkShort (0,[]) nodesWithVars
    mkShort :: (Node, [[Char]]) -> (Node, Maybe C.ByteString) -> ((Node, [[Char]]), (Node, [Char]))
    mkShort idxn (n,Just varName)   = (idxn,(n,C.unpack varName))
    mkShort (idx,notes) (n,Nothing) = ((n,("<"++show idx ++ "> = "++(pretty (e,n))):notes)
                                      ,(n,"<"++show idx ++ ">"))
sczInputs e n = case I.lookup n e of
                 Just (Op _ (SCZ _) inputs) -> Set.unions ((Set.fromList inputs) :
...
node2dot nodeSubs e (ns,sczs) n
  = trace (show ns) $ if Set.member n ns
                       then ((ns,[]),[])
                       else let nns = Set.insert n ns
                         in case I.lookup n e of
                              Nothing -> ((nns,[]),[nnl n (show n ++ " not found")])
                              Just (Op _ op inputs) ->
                                let uniqueInputs = trace (show op) $ L.nub $ L.sort inputs
                                    ((nsNew,sczs'),ls) = L.mapAccumR (node2dot nodeSubs e) (nns,sczs) inputs
                                in ((nsNew
                                    ,(extractSczs op inputs) ++ sczs')
                                   , (nnl n (showOp op nodeSubs)) :
                                        ((concat ls ++ map (en n) uniqueInputs)) )
                              Just (Var _ name) -> ((nns,[]),[nnl n (C.unpack name)])
                              Just (DVar _ name) -> ((nns,[]),[nnl n ("d("++C.unpack name++")")])
                              Just (RelElem i _ idx) -> ((nns,[]),[nnl n ("RE"++show i++show idx)])
                              Just (Const _ d) -> ((nns,[]),[nnl n (show d)])

extractSczs (SCZ (Expression subN subE)) inputs = error $ show [((subE,subN),inputs)]
...
scalarSCZSearch (Scalar (Expression n e)) =
  let
    ...
    fwdRevLookup = (I.fromList $ zip [0..] allInputs
                   ,M.fromList $ zip allInputs [0..]
                   )
    polyEnv = PolyEnv fwdRevLookup (Expression n e)
    sczToPoly n = case I.lookup n e of
                    Just (Op _ (SCZ (Expression sczN sczE)) inputs) -> [(n,extractPoly polyEnv inputs sczE sczN)]
                    x -> error $ "sczsToIndexedInputs found "++show(x,n,e)
  in
    concatMap sczToPoly sczNodes

sczSearch e n = case I.lookup n e of
                  Just (Op _ (SCZ _) _) -> [n]
                  Just (Op _ _ inputs) -> concatMap (sczSearch e) inputs
                  _ -> []

sczsToIndexedInputs e n = case I.lookup n e of
                  Just (Op _ (SCZ (Expression sczN sczE)) inputs) ->
                    let
                      re2Node = I.fromList $ zip [0..] inputs
                      relElems sn = case I.lookup sn sczE of
                                         Just (RelElem num bnd idxs) -> case I.lookup num re2Node of
...
printSCZPolys scalar@(Scalar (Expression n e))
  ...
  where
    ...
    isPoly _           = True
    tvars = case polys of
              (_n,Just (Poly (PolyEnv (fwdLookup,_) _) _)):_ -> concatMap pTvar $ I.toList fwdLookup
              _ -> []
    pTvar (idx,(inNode,bndy,idxs)) = case I.lookup n e of
                                       Just _ -> ["t"++show idx ++ " = " ++ pretty (Expression inNode e) ++ " with " ++ show bndy ++ " offset " ++ show idxs]
                                       _ -> []
    printSCZ (n,_) = show n ++ " = " ++
...
extractPoly pe@(PolyEnv (_,nbidx2varIdx) _envExpr) inputs e n = case I.lookup n e of
  ...
  where
    ...
    varLkup num bnd idxs = case I.lookup num num2Node of
                            Just node -> case M.lookup (node,bnd,idxs) nbidx2varIdx of
                                           Just varIdx -> varIdx
                                           x -> error $ "extractPoly.varLkup2 "++show(x,num,node,bnd,idxs)
                            x -> error $ "extractPoly.varLkup1 "++show(x,num,bnd,idxs)

    --extractMonomial :: Node -> Maybe Monomial
    extractMonomial n = case I.lookup n e of
      ...
      where
        monoFromTerms (vars,c) (term:terms)
          = case I.lookup term e of
              Just (RelElem num bnd idxs) -> monoFromTerms (I.insertWith (+) (varLkup num bnd idxs) 1 vars,c) terms
              Just (Const _ d)  -> monoFromTerms (vars,c*(approxRational d 0.0001)) terms
              _                 -> Nothing
