...
instance Transformable TwoDSparse where
  factor (TwoDSparse sl (Expression n exprs)) = TwoDSparse sl $ Expression n' exprs'
    where
      (exprs',n') = factor' (exprs, n)
  simplify (TwoDSparse sl (Expression n exprs)) = TwoDSparse sl $ Expression n' exprs'
    where
      (exprs',n') = simplify' (exprs, n)
  simplifyOne (TwoDSparse sl (Expression n exprs)) = TwoDSparse sl $ Expression n' exprs'
    where
      (exprs',n') = simplify'' (exprs, n)

instance Transformable TwoDCSparse where
  factor (TwoDCSparse sl (Expression n exprs)) = TwoDCSparse sl $ Expression n' exprs'
    where
      (exprs',n') = factor' (exprs, n)
  simplify (TwoDCSparse sl (Expression n exprs)) = TwoDCSparse sl $ Expression n' exprs'
    where
      (exprs',n') = simplify' (exprs, n)
  simplifyOne (TwoDCSparse sl (Expression n exprs)) = TwoDCSparse sl $ Expression n' exprs'
    where
      (exprs',n') = simplify'' (exprs, n)
...
instance Transformable ThreeDSparse where
  factor (ThreeDSparse sl (Expression n exprs)) = ThreeDSparse sl $ Expression n' exprs'
    where
      (exprs',n') = factor' (exprs, n)
  simplify (ThreeDSparse sl (Expression n exprs)) = ThreeDSparse sl $ Expression n' exprs'
    where
      (exprs',n') = simplify' (exprs, n)
  simplifyOne (ThreeDSparse sl (Expression n exprs)) = ThreeDSparse sl $ Expression n' exprs'
    where
      (exprs',n') = simplify'' (exprs, n)

instance Transformable ThreeDCSparse where
  factor (ThreeDCSparse sl (Expression n exprs)) = ThreeDCSparse sl $ Expression n' exprs'
    where
      (exprs',n') = factor' (exprs, n)
  simplify (ThreeDCSparse sl (Expression n exprs)) = ThreeDCSparse sl $ Expression n' exprs'
    where
      (exprs',n') = simplify' (exprs, n)
  simplifyOne (ThreeDCSparse sl (Expression n exprs)) = ThreeDCSparse sl $ Expression n' exprs'
    where
      (exprs',n') = simplify'' (exprs, n)
