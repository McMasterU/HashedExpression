...
instance Num Construct where
  ...
  x + y = \ e -> let  (e',x') = x e  --FIXME why not sumE [x,y]
                      (e'',y') = y e'
...
instance Fractional Construct where
  x / y = \ e -> let  (e',x') = x e
                      (e'',y') = y e'
                 in if Dim0 == getDimE e'' y' then addEdge e'' $ Op Dim0 Div [x',y']
...
instance RealVectorSpace Construct Construct where
  ...
  mapR _fun _n = error "mapR not implemented for unsafe interface"
     ...
     -}
  projSS ss v = \ e -> let  (e',v') = v e
                            inDim@(Dim2 (d1,d2)) = getDimE e' v'
                            outDims = case ss of
                                        SSCrop _bounds dims -> if dims == [d1,d2] then projDim inDim ss
                                                                                 else error $ "HC.projSS adding "++show (d1,d2,ss) ++ "\n" ++pretty (e',v')
                                        _ -> trace ("projSS ok "++show (d1,d2,ss)) $ projDim inDim ss
