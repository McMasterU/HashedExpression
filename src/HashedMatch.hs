...
mAll consumers match exprs node 
  = case I.lookup node consumers of
      Nothing -> error $ "mAll " ++ show (node,consumers,pretty (exprs, node))
...
mOne consumers match exprs node 
  = case liftM Set.toList $ I.lookup node consumers of
      Nothing -> error $ "mOne " ++ show (node,consumers,pretty (exprs, node))
      Just [consumer] -> match exprs consumer
      Just _ -> Nothing
mAny consumers match exprs node 
  = case I.lookup node consumers of
      Nothing -> error $ "mAny " ++ show (node,consumers,pretty (exprs, node))
      Just nodes -> case catMaybes $ P.map (match exprs) $ Set.toList nodes of
                      (a:_) -> Just a
                      _ -> Nothing
{-
...
o match1 match2 e n = case match1 e n of
                        ...
                        _ -> Nothing
o1 match1 match2 e n = case match1 e n of
                        Just (a,n2) -> let (e',n2') = n2 e in liftM (a,) $ match2 e' n2'
                        _ -> Nothing
o11 match1 match2 e n = case match1 e n of
                        Just (a,n2) -> let (e',n2') = n2 e
                                       in case match2 e' n2' of
                                               Just (b,n3) -> Just ((a,b),n3)
                                               Nothing -> Nothing
                        _ -> Nothing

oReIm match2 e n = case reIm e n of
                     Just (re,im) -> let (_,re') = re e
                                         (_,im') = im e
