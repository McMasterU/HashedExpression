...
data LinearCombination a = LC a (Map.Map String a)
  deriving (Eq,Ord)

instance (Show a, Eq a, Num a, Ord a) => Show (LinearCombination a) where
  show (LC c combos) = case (c,null nonConst) of
                         (0,True) -> "0"
                         (0,_)    -> drop 3 nonConst
                         _        -> show c ++ " + " ++ nonConst
    where
...
instance (Num a, Show a, Eq a, Ord a) => Num (LinearCombination a) where
    (LC c combos) + (LC c' combos') = LC (c+c') (Map.unionWith (+) combos combos')
    ...
    (LC c combos) * (LC c' combos')
      ...
      | Map.null combos' = LC (c*c') (Map.map (*c') combos )
      | True             = error $ "can't form "++show (LC c combos)++" * "++show (LC c' combos')
...
data PositiveCombination a = PC (Map.Map [String] a)
  deriving (Eq,Ord)
...
instance (Num a, Show a, Eq a, Ord a) => Num (PositiveCombination a) where
    (PC combos) + (PC combos') = PC (Map.filter (/= 0) $ Map.unionWith (+) combos combos')
    negate (PC combos) = PC (Map.map negate combos)
    fromInteger x = PC $ Map.singleton [] (fromInteger x)
    (PC combos) * (PC combos') = 
      PC $ Map.filter (/= 0) $ Map.fromListWith (+)  $ filter ((/=0) . snd) 
                                    [(L.sort $ a ++ b,ac*bc)
