...
intDimCheck internal = internal `deepseq` sub 1000 internal $ intDimCheck' internal
  where
    sub 0 _oldIn internal = error $ "intDimCheck' ran out of iterations without reaching fixed point " ++ show internal
    sub iter oldIn internal =  mt ("sub "++ L.intercalate " " [show iter, take 100 (show internal)]) $ if internal == oldIn 
...
-}
intDimCheck' :: Internal -> Internal
intDimCheck' internal = upInternal (I.keys internal) $ downInternal (I.keys internal) internal
{-
...
downEdge node internal = mt ("HDC.downEdge replacing " ++ show currEdge ++ " with " ++ show newEdge) newEdge
  where 
    newEdge = case currEdge of 
{-
\begin{description}
...
-}
      Op (CDims dims) (Compound int) inputs 
        -> Op (CDims (zipWith inputDims (map (\x -> [x]) inputs) dims)) (Compound int) inputs  
      Op DimUnknown (Compound int) inputs   
        -> Op (CDims (zipWith inputDims (map (\x -> [x]) inputs) (repeat DimUnknown))) (Compound int) inputs
{-
...
-- the compound node below did not tell anything, oldEDim is good
          ...
          CDims inDims -> 
            let 
              newEDim = if ((L.length inDims) > int) then mt (show inDims ++ show int)(inDims !! int) else error ("Extract " ++ show int ++ " applied to Compound " ++ show inDims ++ "; the index is probably out of range.")  
            in
            if (newEDim == oldEDim) || (newEDim == DimUnknown)
            then Op oldEDim (Extract int) [input]
            else if oldEDim == DimUnknown 
                 then Op newEDim (Extract int) [input]
                 else error ("HDC.downEdge found: " ++ show currEdge ++ " has child " ++ show (fromJust (I.lookup input internal)))                          
          badCDims     -> error ("HDC.downEdge found node below Extract (should be a Compound) with " ++ show badCDims)
...
-}
      Op dim opId inputs -> case opId of
{-
...
-}
        Compound _     -> error ("HDC.downEdge found Compound node with " ++ show dim)
        Extract _      -> error ("HDC.downEdge found Extract node with " ++ show inputs)
{-
...
-}
        x | elem x [Dot,Prod,Div,Sqrt,Sin,Cos,Tan,Exp,Log,Sinh,Cosh,
                    Tanh,Asin,Acos,Atan,Asinh,Acosh,Atanh,Abs,Signum] 
...
-}
        x | elem x [Sum,Neg,RealPart,ImagPart,RealImag,SubMask,NegMask]
...
-}
        Transpose swap -> Op (revTrDims swap (newDim inputs (revTrDims swap dim))) opId inputs
{-
  \item [Sparse Convolution Zip] same dimensions as the inputs, make sure to also check the innner expression
-}
        SCZ expr       -> Op (newDim inputs dim) (SCZ (exprDimCheck expr)) inputs
{-
...
-}
        ...
        _              -> error ("HDC.downEdge does not (yet) know about " ++ show opId)
{-
...
-}
    ...
    inputDims inputs olDim = 
      case L.filter (/=DimUnknown) (L.nub (downDims node internal inputs [olDim])) of
        []    -> DimUnknown
        [dim] -> dim
        dims  -> error ("HDC.downEdge found: " ++ show currEdge ++ " has child(ren) " ++ show (map fromJust (zipWith I.lookup inputs (repeat internal))) ++ ", which does not match.")
...
downDims node internal (input:inputs) dims 
  = mt ("applying downDims " ++ show node ++ take 100 (show internal) ++ show inputs) 
  (newDim ++ (downDims node internal inputs dims))
    where 
      newDim = case currChild of 
...
upEdge node internal = newEdge
  where 
    newEdge      = case currEdge of  --map a function which takes a dim and returns a dim
      Op DimUnknown (Compound int) inputs ->  --may have to figure this out directly, rather than using newDim
        Op (CDims (newCDims node internal currParents (replicate int DimUnknown))) (Compound int) inputs
      Op (CDims dims) (Compound int) inputs -> 
        Op (CDims (newCDims node internal currParents dims)) (Compound int) inputs 
...
--        Op (CDims (zipWith newDim (map (\x -> [x]) currParents) (repeat DimUnknown)) (Compound int) inputs
    ...
    newDim parents olDim = case parentDims parents [olDim] of
                     []    -> DimUnknown
                     [dim] -> dim
                     dims  -> error ("HDC.upEdge found " ++ show currEdge ++ " has parent(s) " ++ show (map fromJust (zipWith I.lookup parents (repeat internal))) ++ ", which does not match.")
    ...
    newCDims node internal (par:pars) dims = 
      case fromJust (I.lookup par internal) of 
        ...
        Op dim (Extract int) _ -> --say what index are we taking? FIXME
          if length dims > int then --check !! for safety
            if (dims !! int == dim) || (dims !! int == DimUnknown) --check at these match properly
            then newCDims node internal pars (map ((!!) dims) [0..int-1] ++ [dim] ++ map ((!!) dims) [int+1..(length dims-1)])  --take of a list
            else error ("HDC.upEdge found " ++ show (fromJust (I.lookup par internal)) ++ " with child " ++ show (fromJust (I.lookup node internal)))
          else error ("For some reason, we have a problem." ++ show int ++ show dims)
...
-}
        ...
        other -> 
          error ("HDC.upEdge found " ++ show other ++ " with child " ++ show (fromJust (I.lookup node internal)))
        
    parentDims parents olDim = L.filter (/=DimUnknown) (L.nub (upDims node internal parents olDim))
{-
...
upDims node internal (par:pars) dims = upDims node internal pars (dims ++ newDim)
  where 
    newDim = case currParent of
{-
\begin{description}
...
-}
      Op dim ScaleV inputs                  ->    -- case node of (inputs !! 0) -> ; (inputs !! 1) -> ; does this work? then we can raise errors if there are too many inputs to the scaleV
        if mt "!! here 310" (inputs !! 0) == node then [Dim0]
        else [dim]
{-
...
-}
      Op (CDims inDims) (Compound _) inputs -> mt "looking up dim" [inDims !! (fromJust (L.elemIndex node inputs))]
{-
  \item [Extract] the current node is a compound, we should have dealt with this already
-}
      Op dim (Extract int) [input]          -> error ("HDC.upDims found Extract; Extract should not make it through upDims")
      Op dim (Extract int) inputs           -> error ("HDC.upDims found Extract with children " ++ show inputs)
      Op dim opId inputs                    ->
        case opId of
{-
We cannot tell the dimension from the parent, but we need to check the siblings.
-}
          Dot                   -> if length sibDims == 1 || length sibDims == 0 
                                   then sibDims 
                                   else error ("Dot product with input dims " ++ show sibDims)
{-
These functions have the same dimensions as their parents.
-}
          x | elem x [Sum,Neg,RealPart,ImagPart,RealImag,SubMask,NegMask] -> [dim]
          FT _                  -> [dim]
          PFT _ _               -> [dim]
          MapND _ _             -> [dim]
          SCZ _                 -> [dim]
...
-}
          x | elem x [Prod,Div,Sqrt,Sin,Cos,Tan,Exp,Log,Sinh,Cosh,Tanh,Asin,Acos,Atan,Asinh,Acosh,Atanh,Abs,Signum] -> [Dim0]
{-
...
-}
          ...
          _                     -> error ("HDC.upDims does not (yet) know about this Op: " ++ show opId)
      _                                   -> error ("HDC.upDim does not (yet) know about this parent: " ++ show currParent)
{-
\end{description}
-}
...
getSiblings internal node = concat (map (getChildren internal) (I.keys (I.filter (isDotParent node) internal)))
...
-}
exprUpCheck (Expression top internal) = Expression top (upInternal (topSort internal top) internal)
exprDownCheck (Expression top internal) = Expression top (downInternal (topSort internal top) internal)
{-
...
[x2,y2,z2,u2,v2,w2] = map (var2d (3,4)) ["X2","Y2","Z2","U2","V2","W2"]
[x3,y3,z3,u3,v3,w3] = map (var3d (2,3,4)) ["X3","Y3","Z3","U3","V3","W3"]
{-


-}
inputCheck0_0 = getParents (I.fromList [(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,120])]) 120 
inputCheck0_1 = getParents 
{-

Testing some specific pattern matches in upDims
-}
upDims0_1 = upDims 10 (I.fromList [(10, Op (CDims [DimUnknown]) (Compound 1) [5]), (20, Op DimUnknown (Extract 0) [10]), (5, Var Dim0 (p "x1"))]) [20] []
{-
Good, so the parent matches the child, as expected
-}
upDims0_2 = upDims 10 (I.fromList [(10, Op (CDims [Dim0]) (Compound 1) [5]), (20, Op Dim0 (Extract 0) [10]), (5, Var Dim0 (p "x1"))]) [20] []
{-

Here is the problem; HDC.upDims extract node dim Dim1 4 matches compound child dim Dim1 4 
-}
upDims0_3 = upDims 10 (I.fromList [(10, Op (CDims [Dim1 4]) (Compound 1) [5]), (20, Op Dim0 (Extract 0) [10]), (5, Var Dim0 (p "x1"))]) [20] []
{-

this is bad; HDC.upDims extract node dim DimUnknown matches compound child dim DimUnknown
-}
upDims0_4 = upDims 10 (I.fromList [(10, Op (CDims [DimUnknown]) (Compound 1) [5]), (20, Op Dim0 (Extract 0) [10]), (5, Var Dim0 (p "x1"))]) [20] []
{-
...
Test them all:
-}
regressionDimCheck = sequence [putStrLn "dimCheck0_0", print dimCheck0_0,
                               putStrLn "dimCheck0_1", print dimCheck0_1,
                               putStrLn "dimCheck0_2", print dimCheck0_2,
                               putStrLn "dimCheck0_3", print dimCheck0_3,
                               putStrLn "dimCheck0_4", print dimCheck0_4,

                               putStrLn "dimCheck1_0", print dimCheck1_0,
                               putStrLn "dimCheck1_1", print dimCheck1_1,
                               putStrLn "dimCheck1_2", quickCheck dimCheck1_2,
                               
                               putStrLn "dimCheck2_0", print dimCheck2_0,
                               putStrLn "dimCheck2_1", quickCheck dimCheck2_1,
                               putStrLn "dimCheck2_2", print dimCheck2_2,
                               putStrLn "dimCheck2_3", print dimCheck2_3,

                               putStrLn "dimCheck3_0", print dimCheck3_0,
                               putStrLn "dimCheck3_1", quickCheck dimCheck3_1,
                               putStrLn "dimCheck3_2", quickCheck dimCheck3_2,
                               putStrLn "dimCheck3_3", print dimCheck3_3,
                               putStrLn "dimCheck3_4", print dimCheck3_4,
                               putStrLn "dimCheck3_5", print dimCheck3_5,
                               putStrLn "dimCheck3_6", print dimCheck3_6,
                               putStrLn "dimCheck3_7", print dimCheck3_7,
                               putStrLn "dimCheck3_8", print dimCheck3_8,
                               putStrLn "dimCheck3_9", quickCheck dimCheck3_9,
                                                 
                               putStrLn "dimCheck4_0", print dimCheck4_0,
                               putStrLn "dimCheck4_1", print dimCheck4_1,

                               -- putStrLn "dimCheck5_0", print dimCheck5_0,
                               -- putStrLn "dimCheck5_1", print dimCheck5_1,
                               -- putStrLn "dimCheck5_2", quickCheck dimCheck5_2,

                               putStrLn "dimCheck6_0", print dimCheck6_0,

                               putStrLn "dimCheck7_0", print dimCheck7_0,
                               putStrLn "dimCheck7_1", print dimCheck7_1,
                               putStrLn "dimCheck7_2", print dimCheck7_2,
                               putStrLn "dimCheck7_3", print dimCheck7_3,
                               putStrLn "dimCheck7_4", print dimCheck7_4,
                               putStrLn "dimCheck7_5", print dimCheck7_5
...
-}
dimCheck0_0 = exprDimCheck  (Expression 8185171 (I.fromList [(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,120])])) == Expression 8185171 (I.fromList [(120,Var Dim0 (p "x")),(8185171, Op Dim0 Sum [120,120])])
dimCheck0_0_0 = [Expression 8185171 (I.fromList [(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,120])]), exprDimCheck  (Expression 8185171 (I.fromList [(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,120])])), Expression 8185171 (I.fromList [(120,Var Dim0 (p "x")),(8185171, Op Dim0 Sum [120,120])])]
{-

$x+y$ where sum and y are unknown and x is dim0; it currently fixes the sum, but not the y
-}
dimCheck0_1 = exprDimCheck  (Expression 8185171 (I.fromList [(121,Var DimUnknown (p "y")),(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,121])])) == Expression 8185171 (I.fromList [(121,Var Dim0 (p "y")),(120,Var Dim0 (p "x")),(8185171, Op Dim0 Sum [120,121])])

dimCheck0_1_0 = [(Expression 8185171 (I.fromList [(121,Var DimUnknown (p "y")),(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,121])])), exprDimCheck  (Expression 8185171 (I.fromList [(121,Var DimUnknown (p "y")),(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,121])])), Expression 8185171 (I.fromList [(121,Var Dim0 (p "y")),(120,Var Dim0 (p "x")),(8185171, Op Dim0 Sum [120,121])])]

dimCheck0_1_1 = [pretty (Expression 8185171 (I.fromList [(121,Var DimUnknown (p "y")),(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,121])])), pretty (exprDimCheck  (Expression 8185171 (I.fromList [(121,Var DimUnknown (p "y")),(120,Var Dim0 (p "x")),(8185171, Op DimUnknown Sum [120,121])]))), pretty (Expression 8185171 (I.fromList [(121,Var Dim0 (p "y")),(120,Var Dim0 (p "x")),(8185171, Op Dim0 Sum [120,121])]))]
{-

$x+y+z$ where $x+y$, x and y are unknown and x is dim0
-}
dimCheck0_2 = exprDimCheck (Expression 258093934 (I.fromList [(120,Var DimUnknown (p "x")),(121,Var DimUnknown (p "y")),(122,Var Dim0 (p "z")),(8251232,Op DimUnknown Sum [120,121]),(258093934,Op DimUnknown Sum [120,121,122]),(17591436965,Op DimUnknown Sum [8251232,122])])) == (Expression 258093934 (I.fromList [(120,Var Dim0 (p "x")),(121,Var Dim0 (p "y")),(122,Var Dim0 (p "z")),(8251232,Op Dim0 Sum [120,121]),(258093934,Op Dim0 Sum [120,121,122]),(17591436965,Op Dim0 Sum [8251232,122])]))

dimCheck0_2_0 = [exprDimCheck (Expression 258093934 (I.fromList [(120,Var DimUnknown (p "x")),(121,Var DimUnknown (p "y")),(122,Var Dim0 (p "z")),(8251232,Op DimUnknown Sum [120,121]),(258093934,Op DimUnknown Sum [120,121,122]),(17591436965,Op DimUnknown Sum [8251232,122])])), (Expression 258093934 (I.fromList [(120,Var Dim0 (p "x")),(121,Var Dim0 (p "y")),(122,Var Dim0 (p "z")),(8251232,Op Dim0 Sum [120,121]),(258093934,Op Dim0 Sum [120,121,122]),(17591436965,Op Dim0 Sum [8251232,122])]))]
{-

-}
dimCheck0_3 = exprDimCheck (Expression 258093934 (I.fromList [(120,Var DimUnknown (p "x")),(121,Var DimUnknown (p "y")),(122,Var Dim0 (p "z")),(258093934,Op DimUnknown Sum [120,121,122])])) == Expression 258093934 (I.fromList [(120,Var Dim0 (p "x")),(121,Var Dim0 (p "y")),(122,Var Dim0 (p "z")),(258093934,Op Dim0 Sum [120,121,122])])
...
$x+x$ where sum is unknown and x is dim0
-}
dimCheck0_4 = exprDimCheck (Expression 8185171 (I.fromList [(120,Var DimUnknown (p "x")),(8185171, Op Dim0 Sum [120,120])])) == Expression 8185171 (I.fromList [(120,Var Dim0 (p "x")),(8185171, Op Dim0 Sum [120,120])])
dimCheck0_4_0 = [Expression 8185171 (I.fromList [(120,Var DimUnknown (p "x")),(8185171, Op Dim0 Sum [120,120])]), exprDimCheck  (Expression 8185171 (I.fromList [(120,Var DimUnknown (p "x")),(8185171, Op Dim0 Sum [120,120])])), Expression 8185171 (I.fromList [(120,Var Dim0 (p "x")),(8185171, Op Dim0 Sum [120,120])])]
{-
...
-}
dimCheck1_0 = exprDimCheck (Expression 206131415887 (I.fromList [(120,Var DimUnknown (p "x")),(1989048,Var DimUnknown (p "Y1")),(206131415887,Op (Dim1 4) ScaleV [120,1989048])])) == Expression 206131415887 (I.fromList [(120,Var Dim0 (p "x")),(1989048,Var (Dim1 4) (p "Y1")),(206131415887,Op (Dim1 4) ScaleV [120,1989048])]) 
{-

x*y1 with x and *. unknown and y1 known
-}
dimCheck1_1 = exprDimCheck (Expression 206131415887 (I.fromList [(120,Var DimUnknown (p "x")),(1989048,Var (Dim1 4) (p "Y1")),(206131415887,Op DimUnknown ScaleV [120,1989048])])) == Expression 206131415887 (I.fromList [(120,Var Dim0 (p "x")),(1989048,Var (Dim1 4) (p "Y1")),(206131415887,Op (Dim1 4) ScaleV [120,1989048])]) 
{-
x*y1 with x as (Dim1 1); should raise an exception
-}
dimCheck1_2 = exprDimCheck (Expression 206131415887 (I.fromList [(120,Var (Dim1 1) (p "x")),(1989048,Var (Dim1 4) (p "Y1")),(206131415887,Op DimUnknown ScaleV [120,1989048])])) == Expression 206131415887 (I.fromList [(120,Var Dim0 (p "x")),(1989048,Var (Dim1 4) (p "Y1")),(206131415887,Op (Dim1 4) ScaleV [120,1989048])])
{-
...
-}
dimCheck2_0 = exprDimCheck (Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (3,4)) RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])) == Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (3,4)) RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])
{-

transpose (x2+:y2) with the input and output both being (3,4) This should break it.
-}
dimCheck2_1 = exprDimCheck (Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (4,3)) RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])) == Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (4,3)) RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])
{-

transpose (x2+:y2) with everything unknown, except the transpose
-}
dimCheck2_2 = exprDimCheck (Expression 4000889345517 (I.fromList [(2029638,Var DimUnknown (p "X2")),(2029639,Var DimUnknown (p "Y2")),(14873194592,Op DimUnknown RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])) == Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (3,4)) RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])
dimCheck2_2_0 = [exprDimCheck (Expression 4000889345517 (I.fromList [(2029638,Var DimUnknown (p "X2")),(2029639,Var DimUnknown (p "Y2")),(14873194592,Op DimUnknown RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])), Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (3,4)) RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])]
{-
transpose (x2+:y2) with everything unknown, except x2
-}
dimCheck2_3 = exprDimCheck (Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var DimUnknown (p "Y2")),(14873194592,Op DimUnknown RealImag [2029638,2029639]),(4000889345517,Op DimUnknown (Transpose SCR) [14873194592])])) == Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (3,4)) RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])
dimCheck2_3_0 = [exprDimCheck (Expression 4000889345517 (I.fromList [(2029638,Var DimUnknown (p "X2")),(2029639,Var DimUnknown (p "Y2")),(14873194592,Op DimUnknown RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])), Expression 4000889345517 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (3,4)) RealImag [2029638,2029639]),(4000889345517,Op (Dim2 (4,3)) (Transpose SCR) [14873194592])])]
{-
...
-}
dimCheck3_0 = exprDimCheck (Expression 14873194592 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (CDims [Dim2 (3,4),Dim2 (3,4)]) (Compound 2) [2029638,2029639])])) == Expression 14873194592 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (CDims [Dim2 (3,4),Dim2 (3,4)]) (Compound 2) [2029638,2029639])])
{-

Compound (x2,y2) with everything known, but the wrong dims on the compound
-}
dimCheck3_1 = exprDimCheck (Expression 14873194592 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (Dim2 (3,4)) (Compound 2) [2029638,2029639])])) == Expression 14873194592 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (CDims [Dim2 (3,4),Dim2 (3,4)]) (Compound 2) [2029638,2029639])])
{-

Compound (x2,y2) with everything known, but the wrong dims on the compound
-}
dimCheck3_2 = exprDimCheck (Expression 14873194592 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (CDims [Dim2 (3,4),Dim2 (4,3)]) (Compound 2) [2029638,2029639])])) == Expression 14873194592 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(2029639,Var (Dim2 (3,4)) (p "Y2")),(14873194592,Op (CDims [Dim2 (3,4),Dim2 (3,4)]) (Compound 2) [2029638,2029639])])
{-
...
-}
dimCheck3_3 = intDimCheck (I.fromList [(-8550584515149074987,Op (Dim1 4) Sum [-2445111409338921930]),(-3695392959663707949,Op Dim0 Sum [5897069695300716,5897069695300716]),(-2445111409338921930,Op (Dim1 4) ScaleV [1875395714461794112,1989047]),(-1669107634722594519,Op Dim0 Prod [1875395714461794112,5897069695300716]),(1989047,Var (Dim1 4) (p "X1")),(202850972435,Op Dim0 Dot [1989047,1989047]),(1850289671610,DVar (Dim1 4) (p "X1")),(5897069695300716,Op Dim0 Dot [1850289671610,1989047]),(1875395714461753521,Const {unConstDims = Dim0, unConst = 1.0}),(1875395714461794112,Const {unConstDims = Dim0, unConst = 2.0}),(5449969129541664097,Op Dim0 Prod [5897069695300716,1875395714461794112]),(6387268944568490653,Op (CDims [Dim0,CDims [Dim1 4]]) (Compound 2) [202850972435,7341716085073873767]),(7341716085073873767,Op (CDims [Dim1 4]) (Compound 1) [-2445111409338921930])]) == (I.fromList [(-8550584515149074987,Op (Dim1 4) Sum [-2445111409338921930]),(-3695392959663707949,Op Dim0 Sum [5897069695300716,5897069695300716]),(-2445111409338921930,Op (Dim1 4) ScaleV [1875395714461794112,1989047]),(-1669107634722594519,Op Dim0 Prod [1875395714461794112,5897069695300716]),(1989047,Var (Dim1 4) (p "X1")),(202850972435,Op Dim0 Dot [1989047,1989047]),(1850289671610,DVar (Dim1 4) (p "X1")),(5897069695300716,Op Dim0 Dot [1850289671610,1989047]),(1875395714461753521,Const {unConstDims = Dim0, unConst = 1.0}),(1875395714461794112,Const {unConstDims = Dim0, unConst = 2.0}),(5449969129541664097,Op Dim0 Prod [5897069695300716,1875395714461794112]),(6387268944568490653,Op (CDims [Dim0,CDims [Dim1 4]]) (Compound 2) [202850972435,7341716085073873767]),(7341716085073873767,Op (CDims [Dim1 4]) (Compound 1) [-2445111409338921930])])
{-


Just making sure that this expression is okay.
-}
dimCheck3_4 = exprDimCheck (Expression (-8550584515149074987) (I.fromList [(-8550584515149074987,Op (Dim1 4) Sum [-2445111409338921930]),(-3695392959663707949,Op Dim0 Sum [5897069695300716,5897069695300716]),(-2445111409338921930,Op (Dim1 4) ScaleV [1875395714461794112,1989047]),(-1669107634722594519,Op Dim0 Prod [1875395714461794112,5897069695300716]),(1989047,Var (Dim1 4) (p "X1")),(202850972435,Op Dim0 Dot [1989047,1989047]),(1850289671610,DVar (Dim1 4) (p "X1")),(5897069695300716,Op Dim0 Dot [1850289671610,1989047]),(1875395714461753521,Const {unConstDims = Dim0, unConst = 1.0}),(1875395714461794112,Const {unConstDims = Dim0, unConst = 2.0}),(5449969129541664097,Op Dim0 Prod [5897069695300716,1875395714461794112]),(6387268944568490653,Op (CDims [Dim0,CDims [Dim1 4]]) (Compound 2) [202850972435,7341716085073873767]),(7341716085073873767,Op (CDims [Dim1 4]) (Compound 1) [-2445111409338921930])])) == Expression (-8550584515149074987) (I.fromList [(-8550584515149074987,Op (Dim1 4) Sum [-2445111409338921930]),(-3695392959663707949,Op Dim0 Sum [5897069695300716,5897069695300716]),(-2445111409338921930,Op (Dim1 4) ScaleV [1875395714461794112,1989047]),(-1669107634722594519,Op Dim0 Prod [1875395714461794112,5897069695300716]),(1989047,Var (Dim1 4) (p "X1")),(202850972435,Op Dim0 Dot [1989047,1989047]),(1850289671610,DVar (Dim1 4) (p "X1")),(5897069695300716,Op Dim0 Dot [1850289671610,1989047]),(1875395714461753521,Const {unConstDims = Dim0, unConst = 1.0}),(1875395714461794112,Const {unConstDims = Dim0, unConst = 2.0}),(5449969129541664097,Op Dim0 Prod [5897069695300716,1875395714461794112]),(6387268944568490653,Op (CDims [Dim0,CDims [Dim1 4]]) (Compound 2) [202850972435,7341716085073873767]),(7341716085073873767,Op (CDims [Dim1 4]) (Compound 1) [-2445111409338921930])])
{-
...
-}
dimCheck3_5 = exprDimCheck (Expression (-8550584515149074987) (I.fromList [(-8550584515149074987,Op (Dim1 4) Sum [-2445111409338921930]),(-3695392959663707949,Op Dim0 Sum [5897069695300716,5897069695300716]),(-2445111409338921930,Op (Dim1 4) ScaleV [1875395714461794112,1989047]),(-1669107634722594519,Op Dim0 Prod [1875395714461794112,5897069695300716]),(1989047,Var (Dim1 4) (p "X1")),(202850972435,Op Dim0 Dot [1989047,1989047]),(1850289671610,DVar (Dim1 4) (p "X1")),(5897069695300716,Op Dim0 Dot [1850289671610,1989047]),(1875395714461753521,Const {unConstDims = Dim0, unConst = 1.0}),(1875395714461794112,Const {unConstDims = Dim0, unConst = 2.0}),(5449969129541664097,Op Dim0 Prod [5897069695300716,1875395714461794112]),(6387268944568490653,Op DimUnknown (Compound 2) [202850972435,7341716085073873767]),(7341716085073873767,Op DimUnknown (Compound 1) [-2445111409338921930])])) == Expression (-8550584515149074987) (I.fromList [(-8550584515149074987,Op (Dim1 4) Sum [-2445111409338921930]),(-3695392959663707949,Op Dim0 Sum [5897069695300716,5897069695300716]),(-2445111409338921930,Op (Dim1 4) ScaleV [1875395714461794112,1989047]),(-1669107634722594519,Op Dim0 Prod [1875395714461794112,5897069695300716]),(1989047,Var (Dim1 4) (p "X1")),(202850972435,Op Dim0 Dot [1989047,1989047]),(1850289671610,DVar (Dim1 4) (p "X1")),(5897069695300716,Op Dim0 Dot [1850289671610,1989047]),(1875395714461753521,Const {unConstDims = Dim0, unConst = 1.0}),(1875395714461794112,Const {unConstDims = Dim0, unConst = 2.0}),(5449969129541664097,Op Dim0 Prod [5897069695300716,1875395714461794112]),(6387268944568490653,Op (CDims [Dim0,CDims [Dim1 4]]) (Compound 2) [202850972435,7341716085073873767]),(7341716085073873767,Op (CDims [Dim1 4]) (Compound 1) [-2445111409338921930])])
{-
Known compound nodes, everything else is unknown.
-}
dimCheck3_6 = exprDimCheck (Expression (-8550584515149074987) (I.fromList [(-8550584515149074987,Op DimUnknown Sum [-2445111409338921930]),(-3695392959663707949,Op DimUnknown Sum [5897069695300716,5897069695300716]),(-2445111409338921930,Op DimUnknown ScaleV [1875395714461794112,1989047]),(-1669107634722594519,Op DimUnknown Prod [1875395714461794112,5897069695300716]),(1989047,Var DimUnknown (p "X1")),(202850972435,Op DimUnknown Dot [1989047,1989047]),(1850289671610,DVar DimUnknown (p "X1")),(5897069695300716,Op DimUnknown Dot [1850289671610,1989047]),(1875395714461753521,Const {unConstDims = DimUnknown, unConst = 1.0}),(1875395714461794112,Const {unConstDims = DimUnknown, unConst = 2.0}),(5449969129541664097,Op DimUnknown Prod [5897069695300716,1875395714461794112]),(6387268944568490653,Op (CDims [Dim0,CDims [Dim1 4]]) (Compound 2) [202850972435,7341716085073873767]),(7341716085073873767,Op (CDims [Dim1 4]) (Compound 1) [-2445111409338921930])])) == Expression (-8550584515149074987) (I.fromList [(-8550584515149074987,Op (Dim1 4) Sum [-2445111409338921930]),(-3695392959663707949,Op Dim0 Sum [5897069695300716,5897069695300716]),(-2445111409338921930,Op (Dim1 4) ScaleV [1875395714461794112,1989047]),(-1669107634722594519,Op Dim0 Prod [1875395714461794112,5897069695300716]),(1989047,Var (Dim1 4) (p "X1")),(202850972435,Op Dim0 Dot [1989047,1989047]),(1850289671610,DVar (Dim1 4) (p "X1")),(5897069695300716,Op Dim0 Dot [1850289671610,1989047]),(1875395714461753521,Const {unConstDims = Dim0, unConst = 1.0}),(1875395714461794112,Const {unConstDims = Dim0, unConst = 2.0}),(5449969129541664097,Op Dim0 Prod [5897069695300716,1875395714461794112]),(6387268944568490653,Op (CDims [Dim0,CDims [Dim1 4]]) (Compound 2) [202850972435,7341716085073873767]),(7341716085073873767,Op (CDims [Dim1 4]) (Compound 1) [-2445111409338921930])])
{-
...
-}
dimCheck3_7 = exprDimCheck (Expression 100 (I.fromList [(100, Op DimUnknown (Extract 0) [50]), (50, Op DimUnknown (Compound 2) [10,20]), (10, Var (Dim2 (15,16)) (p "T1")), (20, DVar (Dim2 (22,16)) (p "T2"))])) == Expression 100 (I.fromList [(100, Op (Dim2 (15,16)) (Extract 0) [50]), (50, Op (CDims [Dim2 (15,16),Dim2 (22,16)]) (Compound 2) [10,20]), (10, Var (Dim2 (15,16)) (p "T1")), (20, DVar (Dim2 (22,16)) (p "T2"))])
{-

-}
dimCheck3_8 = exprDimCheck (Expression 100 (I.fromList [(100, Op DimUnknown (Extract 1) [50]), (50, Op DimUnknown (Compound 2) [10,20]), (10, Var (Dim2 (15,16)) (p "T2")), (20, DVar (Dim2 (22,16)) (p "T2"))])) == Expression 100 (I.fromList [(100, Op (Dim2 (15,16)) (Extract 1) [50]), (50, Op (CDims [Dim2 (15,16),Dim2 (22,16)]) (Compound 2) [10,20]), (10, Var (Dim2 (15,16)) (p "T2")), (20, DVar (Dim2 (22,16)) (p "T2"))])
dimCheck3_8_0 = [exprDimCheck (Expression 100 (I.fromList [(100, Op DimUnknown (Extract 1) [50]), (50, Op DimUnknown (Compound 2) [10,20]), (10, Var (Dim2 (15,16)) (p "T2")), (20, DVar (Dim2 (22,16)) (p "T2"))])), Expression 100 (I.fromList [(100, Op (Dim2 (15,16)) (Extract 1) [50]), (50, Op (CDims [Dim2 (15,16),Dim2 (22,16)]) (Compound 2) [10,20]), (10, Var (Dim2 (15,16)) (p "T2")), (20, DVar (Dim2 (22,16)) (p "T2"))])]

{-

Extract (Compound (T1,T2)) 
-}
dimCheck3_9 = exprDimCheck (Expression 100 (I.fromList [(100, Op DimUnknown (Extract 2) [50]), (50, Op DimUnknown (Compound 2) [10,20]), (10, Var (Dim2 (15,16)) (p "T1")), (20, DVar (Dim2 (22,16)) (p "T2"))])) == Expression 100 (I.fromList [(100, Op DimUnknown (Extract 2) [50]), (50, Op DimUnknown (Compound 2) [10,20]), (10, Var (Dim2 (15,16)) (p "T1")), (20, DVar (Dim2 (22,16)) (p "T2"))])
{-
...
-}
dimCheck6_0 = exprDimCheck (Expression 9436064 (I.fromList [(120,Var DimUnknown (p "x")),(121,Var DimUnknown (p "y")),(9436064,Op DimUnknown Prod [120,121])])) == Expression 9436064 (I.fromList [(120,Var Dim0 (p "x")),(121,Var Dim0 (p "y")),(9436064,Op Dim0 Prod [120,121])])
{-
...
dimCheck7_1 = exprDimCheck (unTwoD (projSS subX x2)) == 
  Expression 75096643 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(75096643,Op (Dim2 (2,4)) (Project (SSCrop [(2,3),(0,3)] [3,4])) [2029638])])
dimCheck7_2 = exprDimCheck (unTwoD (projSS subW x2)) == 
  Expression 75096643 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(75096643,Op (Dim2 (3,4)) (Project (SSCrop [(1,3),(0,3)] [3,4])) [2029638])])
...
-}
dimCheck7_3 = exprDimCheck (Expression 75096643 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(75096643,Op (Dim2 (2,4)) (Project (SSCrop [(1,3),(0,3)] [3,4])) [2029638])]))
dimCheck7_4 = exprDimCheck (Expression 75096643 (I.fromList [(2029638,Var DimUnknown (p "X2")),(75096643,Op (Dim2 (2,4)) (Project (SSCrop [(1,3),(0,3)] [3,4])) [2029638])]))
dimCheck7_5 = exprDimCheck (Expression 75096643 (I.fromList [(2029638,Var (Dim2 (3,4)) (p "X2")),(75096643,Op DimUnknown (Project (SSCrop [(1,3),(0,3)] [3,4])) [2029638])]))
{-
...
-}
dimCheck9_0 = exprDimCheck (Expression 8185171 (I.fromList [(120,Var DimUnknown (p "x")),(8185171,Op DimUnknown Sum [120,121]),(121,Var Dim0 (p "x"))]))
{-
\end{document}
