...
module HashedSimplify (simplify',simplify'',simpRewrite,simpRewrite'',xMkReal,xMkImag
                      ,mergeScaledConstsBy,HashedExpression.containsDifferential,isDeepZero
                      ,disassociateScale,constExpr,constExpr',simplifyE,mkSCZ
                      ,simp1, applyOne, mkConst -- for testing
                      ) where
...
simplify' (e,n) = e `deepseq` sub 1000 n $ simplify'' (e,n)
  where
    sub 0 _oldN (e,n) = error $ "simplify' ran out of iterations without reaching fixed point " ++ pretty (Expression n e)
    sub iter oldN (e,n) = {- mt ("sub "++show iter) $ -} if n == oldN
                                                      then {- mt ("simplify'ed "++pretty (e,n))-} (e,n)
...
simpRewrite' reWrite (e,n) = sub 1000 n $ simpRewrite'' reWrite (e,n)
  where
    sub 0 _oldN (e,n) = error $ "simpRewrite' ran out of iterations without reaching fixed point " ++ pretty (Expression n e)
    sub iter oldN (e,n) =  if n == oldN
                                   then tt ("simpRewrite'ed "++ show (n{-,e-})++"\n:::") (e,n)
...
simpRewrite'' reWrite (exprs,node)
  ...
  | Just ei <- applyOne (exprs,node) simp1
  = mt "simp1 has been used" ei

  -- * sum with one summand collapses
  -- - Sum(x) -> x
  | Just [n] <- M.sum exprs node
  = tt "one sum" $ n exprs

  -- * product with one factor collapses
  -- - Prod(x) -> x
  | Just [n] <- M.prod exprs node
  = tt "one prod" $ n exprs
  {--}
...
--
  ...
  = tt "pre-2 consts in prod" $
    let (consts,nonconsts) = List.partition ((Nothing /= ) . M.const exprs) inputs
        newC = mkConst Dim0 $ product $ map ( (\ x -> case x of (Just (c,_)) -> c; _ -> error "spm")
                                 . M.const exprs) consts
  ...
  = let (ones,nonones) = List.partition ((Just (Const Dim0 1) == ) . flip I.lookup exprs) inputs
    in tt ("prod consts " ++ concatMap (pretty . (exprs,)) ones ++ " not " ++ concatMap (pretty . (exprs,)) nonones) $ ( prodE (map sourceNode nonones) ) exprs
  --
  ...
  = tt "pre-distribute prod" $
    let  isSum arg = case I.lookup arg exprs of
            Just (Op _ Sum _) -> True
            _ -> False

         getArgs arg = case I.lookup arg exprs of
    ...
    in tt "distribute prod inside sum" $ (sumE [prodE (map sourceNode $ nodeSort' exprs $ l ++ notSums) | l <- lists]) exprs
  --
  -- * sum with no summands
  -- - Sum() -> error
  | Just [] <- M.sum exprs node
  = error $ "simplify found empty sum"
  {--}
  ...
  = tt "pre-complex sum" $
    let
            inputPair = {-mt (unlines $ map (curry pretty exprs) inputs) $-} zip reImInputs inputs
            (fromReImConstructor,others) = List.partition ((/= Nothing) . fst) inputPair
            (reals,imags) = unzip $ map ( \ (x,_) -> case x of Just u -> u ; _ -> error "simp reim")
                                        fromReImConstructor
            [reSum,imSum] = {-mt (show $ length reals)
                 $-} map sumE [reals,imags]


          in tt (("C sum"++) $ show $ map snd others) $ (sumE ((reSum +: imSum):(map (sourceNode . snd) others)) ) exprs
  ...
  -- - Im ( Proj ( x ) ) -> Proj ( Im ( x ) )
  | Just (ss,x) <- (M.xIm `o` M.proj) exprs node
  = (projSS ss $ xIm x) exprs
 ...
 --
  ...
  = tt "pre-2 consts in sum" $
    let (consts,nonconsts) = List.partition ((Nothing /= ) . M.const exprs) inputs
        newC = mkConst Dim0 $ sum $ map ( (\ x -> case x of (Just (c,_)) -> c; _ -> error "spm2")
                                   . M.const exprs) consts
  ...
  = let  -- sort the nodes accoring to real/complex
         (cplx,real) = List.partition (nodeIsComplex exprs) inputs
 ...
 in tt "decompose complex prod into real" $
     (if null real then realProd +: imagProd
                   else (reals * realProd) +: (reals * imagProd)) exprs
...
{-  | Just (Op _dims Sum summands) <- I.lookup node exprs
  ...
  = let
      (sczE2,sczN2) = recreate (sczE1,sczN1)
  ...
  = let
      Scalar (Expression sczN1 sczE1) = (fromDbl c) * (Scalar sczEx)
    in tt ("constant -> SCZ "++pretty (sczE1,sczN1)) $ addEdge exprs $ mkSCZ dims (Expression sczN1 sczE1) inputs
  ...
  = tt "pre-remove unused inputs" $
    ...
    in tt "remove unused inputs" $ addEdge exprs $ case inputs' of
                        [] -> Const dims 0
  ...
  , justReplaceable <-
       let
         removeNonZeroOffsets (idxSCZNode,Just (sczE,args)) = case lookup idxSCZNode offsets of
           Just _n -> (idxSCZNode,Just (sczE,args))
           Nothing -> (idxSCZNode,Nothing)
         removeNonZeroOffsets x                             = x
       in
         map removeNonZeroOffsets $ zip [0..] $ map (isSCZNode exprs) inputs
  -- just the replaceable ones
  , subSCZs <- filter ((/= Nothing) . snd) justReplaceable
  , not $ null subSCZs
  = tt "pre-SCZ [...sczs...]" $
    let  oldIdxsOfReplaceableRelElems = map fst subSCZs
         rewrittenSubSCZs = map (\ (sczEN,args) -> simpRewrite (reRemap args) sczEN)
         ...
         rewriteArgs arg = I.findWithDefault arg arg assocOldNtoNewN
           where
             ...
             assocOldNtoNewN = I.fromListWith
                                 (\ a b -> if a == b then a else error $ "HS.rewriteArgs "++show(a,b))
             ...
             findNegRelElem (node,RelElem idx _b _o) = case lookup idx assocNegOldIdxNewN of
                                                       Just newN -> [(node,newN)]
                                                       Nothing -> []
             findNegRelElem _ = []
         ...
         reRemap args (RelElem idx b o) =
           if idx < 0 then RelElem (inputToNewIdx (args !! (-idx-1))) b o
                      else RelElem idx b o
         reRemap _ x = x

         inputToNewIdx oldNode = case lookup oldNode (replacedInputs ++ (zip keptInputs [0..])) of
                                   Just idx -> idx
    ...
    in tt "SCZ [...sczs...]" $ addEdge exprs
          $ mkSCZ dims (Expression newSCZN newSCZE) keptInputs
  ...
  , summandSCZDiff <- filter ((/= Nothing).fst)
                      $ zip summandSCZ $ map (HashedExpression.containsDifferential exprs) summands
  , not $ null $ drop 1 $ summandSCZDiff         -- there are two SCZs
  , null $ drop 1 $ filter snd summandSCZDiff    -- but at most 1 has a differential
  = tt "sum of SCZs" $
    let -- get single list of all SCZ inputs
        gatheredInputs = List.nub $ List.sort $ concatMap justArgs summandSCZ
        allArgs = zip gatheredInputs [0..]
        -- make map from old reIdxs to new reIdxs
        oldIdxToNew args idx = case lookup (args !! idx) allArgs of
                                 Just new -> new
                                 Nothing -> error "HS..Sum SCZ"
        reRemap args (RelElem idx b o) =
          if idx < 0 then RelElem (oldIdxToNew args (-idx-1)) b o
                     else RelElem idx b o
        reRemap _ x = x
    ...
    in if null $ filter (==Nothing) summandSCZ
       ...
       -- if there are other summands, create a new Sum node
              ...
              nonSCZs = filter ((==Nothing) . isSCZNode exprs) summands
  ...
  = tt "pre-SCZ [...consts...]" $
    let  constIdxs = filter ((/= Nothing) . snd) consts
         ...
         idxRemap = I.fromList $ (zip inputIdxs (map Left [0..]))
         ...
         (sczE',sczN') = {- mt ("SCZ 1" ++ pretty (sczE,sczN) ++" - " ++ show idxRemap) $ -}  simpRewrite remap (sczE,sczN)
...
disassociateScale multiplier exprs factor
  = let ttt :: Int -> (Internal,(Double,Node)) -> (Internal,(Double,Node))
        ttt idx (e,(d,n)) = mt ("disasociateScale " ++ show idx ++" "++ show d ++" "++ pretty (e,n)) (e,(d,n))
    in case I.lookup factor exprs of
      ...
      Just (Op _dims Prod factors@(_:_)) ->
        ...
        in case I.lookup constN exprs of
             Just (Const Dim0 m) -> case rest of
                                    [] -> (exprs,(multiplier,factor))
                                    [r] -> (exprs,(m*multiplier,r))
                                    _ -> let (e,restN) = addEdge exprs $ Op Dim0 Prod $ reverse rest
...
-}
  | Just (Op dims (SCZ (Expression sczN sczE)) inputs) <- uncurry (flip I.lookup) $ left exprs
  ...
  , isLinearIn sczE (\ n -> case I.lookup n sczE of Just (RelElem i _ _) -> i == (length inputs - 1); _ -> False
     ) sczN
     -- *** for now all the dims are the same, but this doesn't allow folding
  = let
      (e1,right') = right exprs

      -- negate the relative offsets in references to the last array variable
      reflectedExpr = eMap (reMapRE $ reflectN $ dimLength dims - 1) $ Expression sczN sczE

      adjoint = \ e -> reflectedExpr `seq` addEdge e
        (mkSCZ dims (simplifyE "sczPP" reflectedExpr) ((reverse restRev) ++ [right']))
...
isLinearIn exprs isSubexpr head
  ...
  , List.and $ map (isLinearIn exprs isSubexpr) inputs
  , nonConsts <- filter ((False==) . isConstIn exprs isSubexpr . snd) $ zip [0..] inputs
  = case nonConsts of
    [] -> True
    [(idx,_)] -> List.and $ map (isLinearIn sczE ( == sczN)) $ reNodeWithIdx idx sczE sczN
    _ -> False
  -- (linear in) / (const in)
...
reMapRE remapArrays remapNodes ee =
  case ee of
    (RelElem idx b offsets) -> let (idx',b',offsets') = remapArrays (idx,b,offsets)
          in RelElem idx' b' offsets'
    Op d id nodes -> let
                          mapNode node = case I.lookup node remapNodes of
                                                    Just x -> x
                                                    _ -> error $ "reMapRE impossible "++show(node,remapNodes,ee)
                        in
...
eMap fun (Expression head exprs) = Expression newHead finalExprs
  where
    ...
    (reMapper, finalExprs) = List.foldl' (flip $ foldEdge fun) (I.empty,I.empty) df
    newHead = case I.lookup head reMapper of
      Just x -> x
      Nothing -> error $ "eMap impossible" ++ show (head,reMapper,exprs)

    foldEdge :: (IntMap Int {-remap nodes-} -> ExpressionEdge -> ExpressionEdge)
      -> (Node,ExpressionEdge) -> (IntMap Int, Internal) -> (IntMap Int, Internal)
...
mergeScaledConstsBy op (exprs,scaledNodes) = (exprs',combined ++ nonconsts)
  where
    consts = concatMap
         ( \ (m,n) -> case I.lookup n exprs of Just (Const Dim0 x) -> [m*x]; _ -> [] )
         scaledNodes
    nonconsts = filter
         ( \ (_m,n) -> case I.lookup n exprs of Just (Const Dim0 _) -> False; _ -> True)
         scaledNodes
...
collapse exprs op term
  = case I.lookup term exprs of
      Just (Op _dims op1 newTerms) -> if op == op1
