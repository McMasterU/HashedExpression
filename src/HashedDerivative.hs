...
instance Show FGrad where
  show (FGrad exprs _top fun param vars) = "{f = "++p fun
         ++"; params = {"++ (concat $ List.intersperse ", " $ map (show.snd) param) ++"}"
         ++"; vars = {"++ (concat $ List.intersperse ", " $ map (show.snd) vars)  ++"}"
         ++ "}" ++ concatMap pd vars
    where
...
instance Diff Scalar where
  ...
  partDiff name (Scalar (Expression n exprs)) = Scalar $ Expression n' exprs'
    where (exprs',n') = partDiff' (\ dims1 n1 exprs1 _node1 -> if name == n1
                       then if  dims1 == Dim0
                         then addEdge' exprs1 (Const Dim0 1)
                         else error "partDiff can only differentiate with respect to a scalar"
  ...
  diff names (Scalar (Expression n exprs)) = Scalar $ Expression n' exprs'
    where (exprs',n') = partDiff' (\ dims1 n1 exprs1 _node -> if n1 `elem` names
...
mkE node exprs err = case I.lookup node exprs of
  Just _ -> Expression node exprs
  Nothing -> error $ "mkE node not in table \"" ++ err ++ "\"" ++ show (node,exprs)

partDiff' ::  (Dims -> ByteString
                     -> Internal
                     -> Node
                     -> Expression
              ,ExpressionEdge {- must be RelElem -} -> Internal -> Node -> Expression)
  -> Internal
  -> Node
  -> (Internal,Node)
partDiff' (dVar,dRel) exprs node = (exprs',node')
  where
    ...
    Expression node' exprs' = simplify $
      case I.lookup node exprs of
        Nothing -> error $ "partDiff didn't find node " ++ (take 200 $ show (node,exprs))
        ...
        Just (Op dims op inputs) -> let
            ...
            d1Input = let (d1E',node) = case inputs of
                                      (input:[]) -> (partDiff' (dVar,dRel)) exprs input
                                      _ -> error $ "partDiff "++show op++" needs 1 input " ++ (take 200 $ show inputs)
                      in addEdge' d1E' (Op dims op [node])
          in  case op of
              Transpose swap -> error $ "derivative doesn't know how to handle transpose "++show swap
              Reglzr _dk _rk -> error $ "HD.partDiff' Rglzr not implemented"
              GradReglzr _dk _rk -> error $ "HD.partDiff' Rglzr not implemented"
              RealPart -> d1Input
              ImagPart -> d1Input
              RealImag -> case inputs of
                            (i1:i2:[]) -> let (e1,dn1) = (partDiff' (dVar,dRel)) exprs i1
                                              (e2,dn2) = (partDiff' (dVar,dRel)) e1 i2
                                          in addEdge' e2 (Op dims RealImag [dn1,dn2])
                            _ -> error $ "partDiff "++show op++" needs 2 input " ++ (take 200 $ show inputs)

              SubMask -> case inputs of
                            (i1:i2:[]) -> let (e1,dn1) = (partDiff' (dVar,dRel)) exprs i1
                                              (e2,dn2) = (partDiff' (dVar,dRel)) exprs i2
                                          in if isDeepZero e1 dn1
                                             then addEdge' e2 (Op dims NegMask [i1,dn2])
                                             else error $ "partDiff SubMask first arg is not constant "++pretty (exprs,i1)
                            _ -> error $ "partDiff "++show op++" needs 2 input " ++ (take 200 $ show inputs)

              NegMask -> case inputs of
                            (i1:i2:[]) -> let (e1,dn1) = (partDiff' (dVar,dRel)) exprs i1
                                              (e2,dn2) = (partDiff' (dVar,dRel)) exprs i2
                                          in if isDeepZero e1 dn1
                                             then addEdge' e2 (Op dims NegMask [i1,dn2])
                                             else error $ "partDiff NegMask first arg is not constant "++pretty (exprs,i1)
                            _ -> error $ "partDiff "++show op++" needs 2 input " ++ (take 200 $ show inputs)

              Sum ->  let (exprs',nodes) = List.mapAccumR (partDiff' (dVar,dRel)) exprs inputs
                          (exprs'',mergedConsts) = mergeConstsBy (+) (exprs', nodes)
                          nonZeros = filterOut isZero exprs'' mergedConsts
                      in case length nonZeros of
                           0 -> addEdge' exprs (Const Dim0 0)
                           1 -> mkE (head nonZeros) exprs'' $ "pD:Sum " ++ (concatMap (pretty . flip Expression exprs) inputs) ++ " | " ++ (concatMap (pretty . flip Expression exprs') nodes) ++ prettyInput
                           _ -> addEdge' exprs'' (Op dims Sum nonZeros)

              Neg ->  d1Input  -- means $\d(-f) = - \d(f)$  (a common pattern)

              Abs ->  error "partDIff encountered abs() which is not differentiable"

              Signum -> error "partDIff encountered signum() which is not differentiable"

              Prod -> let  (exprs',diffFactors :: [Node]) = List.mapAccumR (partDiff' (dVar,dRel)) exprs inputs
                           diffRest = zip diffFactors $ pickOne inputs [] :: [(Node,(Node,[Node]))]
                           withoutZeros = filter (not . isZero . (flip I.lookup exprs') . fst) diffRest
                           (exprs'',prods) = List.mapAccumR
                               (\ ee (dterm,(_term,others)) ->
                                   let  (ee1,newOps) = mergeConstsBy (*) (ee,dterm:others)
                                        newOps' = case map (flip I.lookup ee1) newOps of
                                                       ((Just (Const Dim0 0)):_) -> Const Dim0 0
                                                       [] -> Const Dim0 0
                                                       [Just (Const Dim0 1)] -> Const Dim0 1
                                                       ((Just (Const Dim0 1)):(Just x):[]) -> x
                                                       ((Just (Const Dim0 1)):_xs) -> Op Dim0 Prod $ tail newOps
                                                       _xs -> Op Dim0 Prod newOps
                                   in   addEdge ee1 newOps'
                               ) exprs' withoutZeros
                      in   case prods of
                             [] -> addEdge' exprs'' (Const Dim0 0)
                             [term] -> Expression term exprs''
                             terms -> addEdge' exprs'' (Op dims Sum terms)

              Div -> let  ((num,denom),(exprs',[diffNum,diffDenom]))
                             = case inputs of
                                (num:denom:[]) -> ((num,denom),List.mapAccumR (partDiff' (dVar,dRel)) exprs [num,denom])
                                _ -> error $ "partDiff Div needs 2 inputs " ++ (take 200 $ show inputs)

                          numS = Scalar $ Expression num exprs'
                          denomS = Scalar $ Expression denom exprs'
                          diffNumS = Scalar $ Expression diffNum exprs'
                          diffDenomS = Scalar $ Expression diffDenom exprs'
                          Scalar e
                             = case diffNumS of
                                 0 -> - (diffDenomS * numS) / (denomS * denomS)
                                 _ -> diffNumS / denomS - (diffDenomS * numS) / (denomS * denomS)

                     in gateScalar Div e

              Sqrt -> gateScalar Sqrt $ let (es',diffN) = partDiff' (dVar,dRel) exprs (head inputs) -- safe via getScalar
                                            (es'',result) = (0.5 * (sourceNode diffN) / (sourceNode node)) es'
...
-}
              Atanh -> gateScalar Atanh $ let (es',diffN) = partDiff' (dVar,dRel) exprs (head inputs) -- safe via getScalar
                                          ...
                                          in e
              Dot -> let  ((x,y),(exprs',[dX,dY]))
                             = case inputs of
                                (x:y:[]) -> ((x,y),List.mapAccumR (partDiff' (dVar,dRel)) exprs [x,y])
                                _ -> error $ "partDiff Dot needs 2 inputs " ++ (take 200 $ show inputs)

                          dXdotY = Scalar $ addEdge' exprs' (Op Dim0 Dot [dX,y])
                          dYdotX = Scalar $ addEdge' exprs' (Op Dim0 Dot [dY,x])

                          Scalar e = dXdotY + dYdotX
                     in e
              MapND _ _ -> error "partDiff unimplemented           MapND ->  "
              SCZ e@(Expression sczNode sczExprs) -> let
                  -- scalar diffs:  diff combining expression by the scalar
                  --                and scale the result of the
                  allVars = variables sczExprs sczNode
                  scalars = map fst $ filter ((==Dim0) . getDimE sczExprs . snd) allVars
                  forEachScalar ex s = (ex''',scaledNode)
                    where
                      (ex',sczNode) =  addEdge ex
                                          (mkSCZ dims (case partDiff s $ Scalar e of Scalar x -> x)
                                              inputs)
                      Expression diffNode ex'' = dVar Dim0 s ex' node
                      (ex''',scaledNode) = addEdge ex'' (Op dims ScaleV
                                                                      [diffNode,sczNode])
                  (exprs',scalarDiffs) = List.mapAccumR forEachScalar exprs scalars
                  ...
                  (exprs'',oneDiffedAtATime::[Maybe Node])
                    = List.mapAccumR chainRule exprs' $ zip [0..] inputs
                  ...
                  chainRule :: Internal -> (Int,Node) -> (Internal,Maybe Node)
                  chainRule e (inputNum,input) = if isDeepZero e' dInput
                                                   then (e,Nothing)
                                                   else (e'',Just scz)
                    where
                      (e',dInput) = partDiff' (dVar,dRel) e input
                      (sczExprs',sczNode') = partDiff' (noDiff,dThisRel) sczExprs sczNode
                      (e'',scz) = addEdge e' $ mkSCZ  dims (mkE sczNode' sczExprs' $ "scz "++prettyInput)
                                                    $ inputs++[dInput]

                      dThisRel (RelElem array bndy reIdx) dTExprs _dTNode = if inputNum == array
                        then addEdge' dTExprs $ RelElem (length inputs) bndy reIdx -- differential in new input
                        else addEdge' dTExprs $ Const Dim0 0 -- this was wrong, mkE dTNode dTExprs $ "leave node alone " ++ prettyInput
                      dThisRel errEE errE errN = error $ "dThisRel found " ++ show (errEE,errE,errN)

                -- then add up all the terms
                in addEdge' exprs'' $ Op dims Sum $ scalarDiffs ++ (catMaybes oneDiffedAtATime)
              ScaleV ->
                let  (exprs' :: Internal ,[x,y,dX,dY]::[Internal -> (Internal,Node)])
                       = case inputs of
                         (x:y:[]) ->
                           let (exprs''::Internal,[dX,dY])=List.mapAccumR (partDiff' (dVar,dRel)) exprs [x,y]
                           in (exprs'',map sourceNode [x,y,dX,dY])
                         _ -> error $ "partDiff ScaleV needs 2 inputs " ++ (take 200 $ show inputs)

                     dims = getDimE exprs node
                     scaleOrProd = if Dim0 == dims then (*) else scale :: Construct -> Construct -> Construct

                in bigE $ ((dX `scaleOrProd` y) + (x `scaleOrProd` dY)) exprs'

              PFT _dir _idx -> d1Input
              FT _dir -> d1Input
              Project _ss -> d1Input
              Inject _ss -> d1Input
              Extract _elem -> case inputs of
                [single] -> case I.lookup single exprs of
                    op -> error $ "partDiff encountered unhandled Extract " ++ show op
                _ -> error $ "partDiff encountered unhandled Extract " ++ show inputs
...
mergeConstsBy op (exprs,nodes@(n1:_)) = (exprs',combined ++ nonconsts)
  where
    consts = concatMap
         ( \ n -> case I.lookup n exprs of Just (Const _ x) -> [x]; _ -> [] )
         nodes
    nonconsts = filter
         ( \ n -> case I.lookup n exprs of Just (Const _ _) -> False; _ -> True)
         nodes
    (exprs',combined) = if null consts  then (exprs,[])
                                        else (\ (a,b) -> (a,[b])) $ addEdge exprs (Const (getDimE exprs n1) $ foldr1 op consts)
...
mapOne' fun fstBack (b:bs) = (reverse fstBack ++ ((fun b) : bs)) : ( mapOne' fun (b:fstBack) bs )
mapOne' _fim _fstBack [] = []
testMapOne = [[11,2,3],[1,12,3],[1,2,13]] == mapOneAtATime (+10) [1..3]
...
mapAccumOne fun s fstBack (b:bs) = let
    (s',b') = fun s b
...
groupedToScaled exprs scaledGroup =
   case sum scalings of
      ...
      1 -> (exprs,node)
      s -> let (e,n) = addEdge exprs (Const Dim0 s)
           in  addEdge e (Op dims  (case dims of
                                      Dim0 -> Prod
                                      _ -> ScaleV
                                    )
                                    [n,node] )
...
aux exprsNoOne inputs = let
        (exprs,one) = addEdge exprsNoOne $ Const Dim0 1
        ...
        extractDiff n
          ...
          = case rest of
              ...
              _ -> error "HD.gatherDiffs found product with differential and more than 2 factors, needs foldr"
        extractDiff n
          | Just (DVar _ _name) <- I.lookup n exprs
          = [(n,Nothing,one)]
        ...
        extractDiff n
          ...
          , Just (DVar _ _name) <- I.lookup left exprs
          = [(left,Just scale,right)]
        extractDiff n = error $ "Error: extractDiff " ++ show n ++ (unlines $ (pretty' (exprs,n))
                : "inputs" : map (pretty' . (exprs,)) inputs)

        unzipDot :: Internal -> [(Node,Maybe Node,Node)] -> (Internal,(Node,Node))
        unzipDot e0 dotNodes@((dX,_,_):_) = (e2, (dX,innerSum))
          where
            ...
            (e2,innerSum) = simplify' $ addEdge e1 (Op (getDimE e0 dX) Sum terms)

            maybeScale e (_dX,Nothing,right) = (e,right)
            maybeScale e (_dx,Just scale,right) = addEdge e $ Op (getDimE e0 dX) ScaleV [scale,right]
...
gatherDiffs1 f (Scalar (Expression n e)) =
  let  (e1,diffGrads) = gatherDiffs (e,n)
       showDiffs e (nodeD,nodeG) = case (getDimE e nodeD,getDimE e nodeG) of
         (Dim1 _,Dim1 _) -> show ((OneD $ Expression nodeD e),(f $ OneD $ Expression nodeG e))
         (Dim2 _,Dim2 _) -> show ((TwoD $ Expression nodeD e),(simplify $ TwoD $ Expression nodeG e))
         (Dim3 _,Dim3 _) -> show ((ThreeD $ Expression nodeD e),(simplify $ ThreeD $ Expression nodeG e))
...
funGrad fun vars = -- trace (show $ map (flip I.lookup eDiffs . fst) listDGrad) $
  ...
  where
    ...
    (eDiffs,listDGradUnordered) = gatherDiffs $ simplify' (eDiff, -- trace (("before extract:"++) $ pretty' $ simplify (eDiff,nDiff))
                                                           nDiff)

    listDGrad = concatMap (\ v -> filter ((isD v) . flip I.lookup eDiffs . fst)
                                         listDGradUnordered )
                          vars

    isD v (Just (DVar _ name)) = name == v
    isD _ _ = error $ "HD.funGrad non-dvar" ++ show (listDGradUnordered,map (flip I.lookup eDiffs . fst) listDGradUnordered)

    Scalar fE = fun

    (eGrad, gradN) = addEdge eDiffs $ Op (CDims (map (getDimE exprComp) (map snd listDGrad))) (Compound $ length listDGrad) $ map snd listDGrad --I'm not sure is exprComp is the right place to find the nodes, but it should work.

    (Expression _ exprComp, (nf,ng)) = merge fE (Expression gradN eGrad)

    (exprFinal, compoundN) = simplify' $ addEdge exprComp $ Op (CDims (map (getDimE exprComp) [nf,ng]))  (Compound 2) [nf,ng] --I'm not sure is exprComp is the right place to find the nodes, but it should work.

    (params,nodeVarsUnsorted :: [(Node,ByteString)])  = List.partition (not . flip elem vars . snd)
                   $ map (\ (name,n) -> (n,name))
                   $ variables exprFinal compoundN

    -- make sure variables are in the same order we were given them
    nodeVars = concatMap (\ v -> filter ((v==).snd) nodeVarsUnsorted) vars

    (funN,listGradN) = case I.lookup compoundN exprFinal of
       Just (Op _ (Compound 2) [funN,listGradN]) -> (funN,listGradN)
       _ -> error "funGrad impossible 1"

    listGradSimp = case I.lookup listGradN exprFinal of
...
isDotDiff exprs n = case I.lookup n exprs of
  Just (Op _ Dot [left,_right]) -> case I.lookup left exprs of
     Just (DVar _ _) -> True
     _ -> False
  Just (Op _ Prod [const,dot]) -> case map (flip I.lookup exprs) [const,dot] of
     [Just (Const Dim0 _),Just (Op _ Dot [left,_right])] -> case I.lookup left exprs of
         Just (DVar _ _) -> True
         _ -> False
     _ -> False
  _ -> False
